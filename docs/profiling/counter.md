...
#### <a name="parameters"></a>Parametry
 `Name` název čítače. Pomocí možnosti VSPerfCmd. exe **/QueryCounters** můžete vypsat názvy dostupných čítačů v počítači.

 `Reload` počet událostí čítače v intervalu vzorkování. Nepoužívejte s metodou instrumentace.

 `FriendlyName` (nepovinný) řetězec, který se má použít místo `Name` v hlavičkách sloupců sestav a zobrazení profileru.

## <a name="required-options"></a>Požadované možnosti
 Možnost čítače lze použít pouze s jednou z následujících možností:

 **Start:** `Trace` inicializuje Profiler, aby používal metodu instrumentace.
