...
3. Na panelu nabídek vyberte možnost **ladit** > **profilování výkonu**.
...
6. Přepněte na Visual Studio stisknutím klávesy **Alt**+**TAB**.
...
## <a name="show-built-in-objects"></a>Zobrazit předdefinované objekty
 ...
 Chcete-li zobrazit tyto objekty, vyberte možnost **Zobrazit předdefinované** v ![rozevíracím&#45;seznamu nastavení seznamu nastavení v části analyzátor paměti](../profiling/media/js_mem_settings.png "JS_Mem_Settings") v pravém horním rohu podokna.
...
- Sledujte pracovní postup, který je popsaný v části [izolování nevracení paměti](#isolate-a-memory-leak) a použití **objektů zbývajících z snímku #\<číslo >** filtru v zobrazení rozdílů k identifikaci pravděpodobných kandidátů na nevracení paměti.
...
- Hledejte objekty, které se nechtěně uchovávají v paměti, poté, co uživatel přejde na novou stránku, což je běžná příčina problémů s pamětí. Příklad:
  ...
  - Některé objekty můžou poskytovat `dispose` metodu a doporučení pro použití. Například byste měli volat `dispose` v [WinJS. Binding. list](/previous-versions/windows/apps/hh700774\(v\=win.10\)) , pokud zavoláte metodu `createFiltered` seznamu a potom opustíte stránku.
