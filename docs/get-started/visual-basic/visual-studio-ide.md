...
1. Otevřete Visual Studio. V okně Start vyberte možnost **pokračovat bez kódu**.
...
2. Na panelu nabídek vyberte možnost **nástroje**  > **Možnosti** . otevře se dialogové okno **Možnosti** .

3. Na stránce **prostředí**  > **Obecné** možnosti změňte výběr **barevného motivu** na **tmavý**a pak zvolte **OK**.
...
7. Dále znovu použijeme refaktoring, aby byl kód trochu výstižnější. V `Dim now = Date.Now` řádku klikněte na proměnnou `now`.
