...
3. Ve třídě projektu vytvořte metodu definující příkaz služby SharePoint. Metoda musí splňovat následující pokyny:
    ...
    - Může mít jeden nebo dva parametry.
         ...
         Prvním parametrem musí být objekt <xref:Microsoft.VisualStudio.SharePoint.Commands.ISharePointCommandContext>. Tento objekt poskytuje Microsoft. SharePoint. SPSite nebo Microsoft. SharePoint. SPWeb, ve kterém se příkaz spustí. Poskytuje také objekt <xref:Microsoft.VisualStudio.SharePoint.Commands.ISharePointCommandLogger>, který lze použít k zápisu zpráv do okna **výstupu** nebo **Seznam chyb** okna v aplikaci Visual Studio.
...
## <a name="example"></a>Příklad
 ...
 Kromě implicitního prvního <xref:Microsoft.VisualStudio.SharePoint.Commands.ISharePointCommandContext> parametru má tento příkaz také vlastní parametr řetězce, který obsahuje úplnou cestu k souboru. wsp, který je upgradován na web služby SharePoint. Chcete-li zobrazit tento kód v kontextu většího příkladu, přečtěte si [Návod: Vytvoření vlastního kroku nasazení pro projekty SharePoint](../sharepoint/walkthrough-creating-a-custom-deployment-step-for-sharepoint-projects.md).
...
## <a name="deploying-the-command"></a>Nasazení příkazu
 Chcete-li nasadit příkaz, zahrňte sestavení příkazu do stejného balíčku rozšíření [!include[vsprvs](../sharepoint/includes/vsprvs-md.md)] (*VSIX*) se sestavením rozšíření, které používá příkaz. Také je nutné přidat položku pro sestavení příkazu v souboru extension. vsixmanifest. Další informace naleznete v tématu [nasazení rozšíření pro nástroje služby SharePoint v aplikaci Visual Studio](../sharepoint/deploying-extensions-for-the-sharepoint-tools-in-visual-studio.md).
