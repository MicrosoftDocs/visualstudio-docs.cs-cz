...
description: Přečtěte si o přírůstkových sestaveních MSBuild, která jsou optimalizovaná tak, aby se neprováděly aktuální výstupní soubory.
...
Sada souborů reprezentovaných `Compile` typem položky je zkopírována do záložního adresáře. Záložní soubory mají příponu názvu souboru *. bak* . Pokud nejsou soubory, které jsou určeny typem položky `Compile`, nebo odpovídající záložní soubory odstraněny nebo změněny po spuštění cíle zálohování, dojde v následujících sestaveních k vynechání cíle zálohování.
...
Pro podporu přírůstkové kompilace musí úkoly zajistit, aby byla hodnota atributu `TaskParameter` jakéhokoli prvku `Output` rovna vstupnímu parametru úkolu. Tady je několik příkladů:
...
Z důvodu odvození výstupu je nutné přidat `CreateProperty` úlohu k cíli pro prohlédnutí vlastností a položek, abyste mohli určit, zda byl cíl proveden. Přidejte úkol `CreateProperty` do cíle a přiřaďte mu prvek `Output`, jehož `TaskParameter` je „ValueSetByTask“.
