---
title: Přírůstková sestavení | Dokumenty společnosti Microsoft
ms.date: 11/04/2016
ms.topic: conceptual
helpviewer_keywords:
...
> [!NOTE]
> Když MSBuild vyhodnotí vstupní soubory, jsou považovány za pouze obsah seznamu v aktuálním spuštění. Změny v seznamu z posledního sestavení automaticky nezařazují cíl.
...
Mapování 1 : 1 jsou obvykle tvořena transformací položek. Další informace naleznete v [tématu Transformace](../msbuild/msbuild-transforms.md).
...
Sada souborů reprezentovaných typem `Compile` položky se zkopíruje do záložního adresáře. Záložní soubory mají příponu *.bak* název souboru. Pokud nejsou soubory, které jsou určeny typem položky `Compile`, nebo odpovídající záložní soubory odstraněny nebo změněny po spuštění cíle zálohování, dojde v následujících sestaveních k vynechání cíle zálohování.
...
- Cíl má zastaralé výstupy a je spuštěn tak, aby byly aktuální.
...
Tento kód vytvoří vlastnost Easy, která má hodnotu "123", zda je cíl proveden nebo přeskočen.

Při spuštění nástroje MSBuild 3.5 se u skupin položek a vlastností v cíli automaticky provede odvození výstupu. `CreateItem`úkoly nejsou v cíli vyžadovány a je třeba se jim vyhnout. Úkoly `CreateProperty` by měly být použity v cíli pouze k určení toho, zda byl cíl spuštěn.

Před MSBuild 3.5, můžete použít [CreateItem](../msbuild/createitem-task.md) úkol.

## <a name="determine-whether-a-target-has-been-run"></a>Určení, zda byl cíl spuštěn

Z důvodu odvození výstupu je třeba `CreateProperty` přidat úkol do cíle, abyste prozkoumali vlastnosti a položky, abyste mohli určit, zda byl cíl proveden. Přidejte úkol `CreateProperty` do cíle a přiřaďte mu prvek `Output`, jehož `TaskParameter` je „ValueSetByTask“.
...
Tento kód vytvoří vlastnost CompileRan a `true`dává mu hodnotu , ale pouze v případě, že cíl je proveden. Pokud je cíl vynechán, nebude vlastnost CompileRan vytvořena.
