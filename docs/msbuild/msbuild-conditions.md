...
Nástroj MSBuild podporuje konkrétní sadu podmínek, které lze použít všude, kde je povolen atribut `Condition`. Tyto podmínky jsou vysvětleny v následující tabulce.
...
|And|Vyhodnotí na `true`, pokud se oba operandy vyhodnotí jako `true`.|
