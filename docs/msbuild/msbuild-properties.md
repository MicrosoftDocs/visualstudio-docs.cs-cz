...
```
 ...
 Globální vlastnosti je také možné nastavit nebo upravit pro podřízené projekty v sestaveních s více projekty pomocí atributu `Properties` úkolu MSBuild. Globální vlastnosti jsou také předávány podřízeným projektům, pokud atribut `RemoveProperties` úlohy MSBuild se nepoužívá k určení seznamu vlastností, které nemají být předány. Další informace najdete v tématu [Úloha MSBuild](../msbuild/msbuild-task.md).
