...
## <a name="in-process-compilers"></a>Vnitroprocesové kompilátory
 Pokud je to možné, [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] se pokusí použít verzi v procesu [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] kompilátoru pro zajištění zvýšeného výkonu. (Netýká se [!INCLUDE[csprcs](../data-tools/includes/csprcs_md.md)].) Aby to fungovalo správně, musí se splnit následující podmínky:
