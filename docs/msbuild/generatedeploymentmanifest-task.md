...
| `MapFileExtensions` | Volitelný parametr `Boolean`.<br /><br /> Určuje, zda je použito mapování přípon názvu souboru *. deploy* . Pokud je tento parametr `true`, všechny programové soubory se publikují s příponou *. deploy* . Tato možnost je užitečná pro zabezpečení webového serveru k omezení počtu přípon názvů souborů, které je nutné odblokovat, aby bylo možné povolit nasazení aplikace ClickOnce. Výchozí hodnota tohoto parametru je `false`. |
