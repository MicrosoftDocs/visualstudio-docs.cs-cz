...
| BuildProjectReferences | Logická hodnota, která určuje, zda jsou odkazy na projekt vytvořeny pomocí [!INCLUDE[vstecmsbuild](../extensibility/internals/includes/vstecmsbuild_md.md)]. Automaticky nastaveno na `false`, pokud sestavíte projekt v integrovaném vývojovém prostředí (IDE) [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)], `true` Pokud jinak. na příkazovém řádku lze zadat `-p:BuildProjectReferences=false`, aby se předešlo tomu, že odkazované projekty budou aktuální. |
...
| ProduceReferenceAssembly | Logická hodnota, která Pokud je nastavena na hodnotu `true` povolí výrobu [referenčních sestavení](/dotnet/standard/assembly/reference-assemblies) pro aktuální sestavení. Při použití této funkce by měla být `Deterministic` `true`. Tato vlastnost odpovídá přepínači `/refout` v kompilátorech *Vbc. exe* a *CSc. exe* . |
