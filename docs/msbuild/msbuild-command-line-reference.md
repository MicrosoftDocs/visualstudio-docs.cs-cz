...
|-graphBuild [:`True` nebo `False`]|-Graph [:`True` nebo `False`]|Způsobí, že nástroj MSBuild sestaví a sestaví graf projektu. Sestavování grafu zahrnuje identifikaci odkazů projektu na závislosti formulářů. Sestavování grafu zahrnuje pokus o sestavení odkazů na projekt před projekty, které na ně odkazují, se liší od tradičního plánování MSBuild.|
...
|– obnovení|-r|Před vytvořením skutečných cílů spustí `Restore` cíl.|
...
|-podrobnost:`level`|-v:`level`|Určuje množství informací, které se mají zobrazit v protokolu sestavení. Každý protokolovací nástroj zobrazuje události na základě úrovně podrobností, kterou jste nastavili pro daný protokolovací nástroj.<br /><br /> Můžete určit následující úrovně podrobností: `q[uiet]`, `m[inimal]`, `n[ormal]` (výchozí), `d[etailed]`a `diag[nostic]`.<br /><br /> Toto nastavení je příklad: `-verbosity:quiet`
