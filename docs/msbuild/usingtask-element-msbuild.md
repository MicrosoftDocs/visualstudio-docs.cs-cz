...
|`TaskFactory`|Nepovinný atribut.<br /><br /> Určuje třídu v sestavení, která je zodpovědná za generování instancí zadaného `Task`ho názvu.  Uživatel může také určit `TaskBody` jako podřízený prvek, který objekt pro vytváření úloh obdrží a použije k vygenerování úlohy. Obsah `TaskBody` je specifický pro objekt pro vytváření úloh.|
