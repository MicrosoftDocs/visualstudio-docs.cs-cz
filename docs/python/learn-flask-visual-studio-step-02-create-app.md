...
1. Ve složce projektu vytvořte složku aplikace s názvem `HelloFlask` (klikněte pravým tlačítkem myši na projekt v **Průzkumník řešení** a vyberte **Přidat** > **Nová složka**.)

2. Ve složce *HelloFlask* vytvořte soubor s názvem *\_\_init\_\_. py* s následujícím obsahem, který vytvoří instanci `Flask` a načte zobrazení aplikace (vytvořené v dalším kroku):
...
3. Ve složce *HelloFlask* vytvořte soubor s názvem *views.py* s následujícím obsahem. Název *views.py* je důležitý, protože jste použili `import HelloFlask.views` v rámci *\_\_init\_\_. py*; Pokud se názvy neshodují, zobrazí se v době běhu chyba.
    ...
    Kromě přejmenování funkce a směrování na `home`obsahuje tento kód kód vykreslování stránky z *App.py* a importuje objekt `app`, který je deklarován v *\_\_init\_\_. py*.
...
1. V **Team Explorer**zadejte potvrzovací zprávu, jako je refaktoring Code, a vyberte **potvrdit vše**. Po dokončení potvrzení se zobrazí **hodnota hash \<potvrzení zprávy > vytvořena místně. Synchronizace pro sdílení změn se serverem.** Pokud chcete doručovat změny do vzdáleného úložiště, vyberte **synchronizovat**a potom v části **odchozí potvrzení**vyberte možnost **push** . Před odesláním do vzdáleného úložiště můžete také shromáždit několik místních potvrzení.
...
Funkce `home`, kterou máte k dispozici v *views.py* , vygeneruje pro stránku nic víc než odpověď HTTP v podobě prostého textu. Většina skutečných webových stránek ale reaguje s bohatou stránkou HTML, která často zahrnuje živá data. Hlavním důvodem pro definování zobrazení pomocí funkce je skutečně generování obsahu dynamicky.
...
1. Chcete-li převést vykreslování stránky na použití šablony, vytvořte soubor s názvem *index. html* ve složce *šablony* s následujícím obsahem, kde `{{ content }}` je zástupný symbol nebo token pro nahrazení (označovaný také jako *proměnná šablony*), pro který jste Zadejte hodnotu v kódu:
...
1. Upravte funkci `home`, aby používala `render_template` k načtení šablony a zadání hodnoty "content", která se provádí pomocí pojmenovaného argumentu, který odpovídá názvu zástupného symbolu. Baňka automaticky vyhledá šablony ve složce *šablony* , takže cesta k šabloně je relativní vzhledem k této složce:
...
1. Spusťte aplikaci, podívejte se na výsledky a sledujte, že vložený kód HTML v `content` hodnota se nevykreslí *jako* HTML, protože modul šablonování (Jinja) automaticky řídí obsah HTML. Automatické uvozovací znaky zabraňují náhodným útokům prostřednictvím injektáže: vývojáři často shromažďují vstupy z jedné stránky a používají je jako hodnotu v jiné prostřednictvím zástupného symbolu šablony. Řídicí uvozovací znaky také slouží jako připomenutí, že je znovu vhodné zachovat kód HTML z kódu.
    ...
    Pak aktualizujte funkci `home`, aby poskytovala hodnoty pro všechny zástupné symboly:
...
Odpověď: Pokud se zobrazí chyby, které se nepovedlo najít, ujistěte se, že jste aplikaci přidali do *Settings.pyy* projektu v seznamu `INSTALLED_APPS`. Bez této položky se v baňce nedozvíte, jak ve složce *šablon* aplikace Hledat.
...
Odpověď: Ano, můžete použít podsložky a potom v rámci *šablon* v voláních `render_template`odkazovat na relativní cestu. Je to skvělý způsob, jak efektivně vytvořit obory názvů pro vaše šablony.
