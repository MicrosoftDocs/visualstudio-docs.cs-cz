...
[![sady Visual Studio s projektem Pythonu](media/tour-ide-overview.png)](media/tour-ide-overview.png#lightbox)
...
- [Refactoring](refactoring-python-code.md)
   ...
   Pokud kliknete pravým tlačítkem na část kódu a vyberete **rychlé akce a refaktoring**, Visual Studio poskytuje operace, jako je například inteligentní přejmenování proměnných, extrakce jednoho nebo více řádků kódu do nové metody, změna pořadí metody parametry a další.
...
- [Interaktivní REPL Pythonu](python-interactive-repl-in-visual-studio.md)
    ...
    Visual Studio poskytuje interaktivní okno pro čtení a vyhodnocení-tisk smyčky (REPL) pro každé prostředí Pythonu, které se vylepšuje na REPL, které získáte pomocí *Python. exe* na příkazovém řádku. V **interaktivním** okně můžete zadat libovolný kód Pythonu a zobrazit okamžité výsledky.

    ![Interaktivní okno Pythonu](media/interactive-window.png)

- [Ladění](debugging-python-in-visual-studio.md)

    Visual Studio poskytuje komplexní ladicí prostředí pro Python, včetně připojení ke spuštěným procesům, vyhodnocování výrazů v okně **kukátko** a **bezprostředních** oknech, kontrolu místních proměnných, zarážek, krokování/převzetí/převzetí služeb při selhání. příkazy, **nastavit další příkaz**a další. Můžete také ladit vzdálený kód Pythonu běžící na počítačích se systémem Linux.

    ![Ladění Pythonu v aplikaci Visual Studio](media/remote-debugging-breakpoint-hit.png)

- [Interakce sC++](working-with-c-cpp-python-in-visual-studio.md)

    Řada knihoven vytvořených pro Python je vytvořena C++ pro zajištění optimálního výkonu. Visual Studio poskytuje rozsáhlá zařízení pro C++ vývoj rozšíření, včetně [ladění ve smíšeném režimu](debugging-mixed-mode-c-cpp-python-in-visual-studio.md).

    ![Ladění v Pythonu ve smíšeném režimu C++ a společně](media/mixed-mode-debugging.png)
