...
> [!WARNING]
> Vyhněte se dlouhému zastavení při zarážce při vzdáleném ladění. Azure považuje proces, který se zastavil déle než několik minut, když přestane reagovat a zastaví odesílání provozu do této instance. Pokud se zastavíte příliš dlouho, msvsmon. exe se odpojí od procesu.
