...
Abychom vám pomohli objasnění těchto konceptů, provedeme si nějaký ukázkový kód, který už obsahuje několik chyb. Používáme C#, ale funkce ladění platí pro Visual Basic, C++, JavaScript, Python a další podporované jazyky.
...
1. Otevřete sadu Visual Studio.
    ...
    V horním řádku nabídek zvolte **Soubor** > **Nový** > **Projekt**. V levém podokně dialogového okna **Nový projekt** , v části **C#vizuál**zvolte **Konzolová aplikace**a pak v prostředním podokně zvolte buď **Konzolová aplikace (.NET Framework)** nebo **aplikace konzoly (.NET Core)** . Zadejte název jako **ConsoleApp-FirstApp** a klikněte na **OK**.
    ::: moniker-end
...
1. V případě, že je aplikace stále spuštěná, nastavte zarážku kliknutím na levý okraj vedle volání metody `Console.WriteLine` v tomto řádku kódu.
...
1. Najeďte myší na proměnnou `GalaxyType` napravo a potom vlevo od ikony klíče rozbalte položku `theGalaxy.GalaxyType`. Vidíte, že `GalaxyType` obsahuje `MyGType`vlastností a hodnota vlastnosti je nastavená na `Spiral`.
...
1. V okně zpráva **Upravit a pokračovat** klikněte na **Upravit** . V okně **Seznam chyb** se teď zobrazí chybová zpráva. Tato chyba označuje, že `'object'` neobsahuje definici pro `MyGType`.

    ![Chyba syntaxe](../debugger/media/beginners-no-definition.png)

    I když nastavíme každý Galaxy s objektem typu `GType` (který má vlastnost `MyGType`), ladicí program nerozpozná objekt `theGalaxy` jako objekt typu `GType`. Co se děje? Chcete si projít libovolným kódem, který nastaví typ Galaxy. Když to uděláte, uvidíte, že `GType` třída má jednoznačně vlastnost `MyGType`, ale něco není napravo. Chybová zpráva o `object` zapíná jako o tom, že je to. pro Interpret jazyka se typ jeví jako objekt typu `object` namísto objektu typu `GType`.

1. Hledáním kódu, který souvisí s nastavením typu Galaxy, se nachází vlastnost `GalaxyType` třídy `Galaxy` je zadána jako `object` namísto `GType`.
...
1. Kliknutím na tlačítko **restartovat** ![aplikaci](../debugger/media/dbg-tour-restart.png "RestartApp") na panelu nástrojů ladění (**CTRL** + **SHIFT** + **F5**) znovu zkompilujete kód a restartujte.

    Teď, když se ladicí program na `Console.WriteLine`pozastaví, můžete ukazatel myši najeďte na `theGalaxy.GalaxyType.MyGType`a podívat se, že hodnota je správně nastavená.

1. Odstraňte zarážku kliknutím na kroužek zarážky na levém okraji (nebo klikněte pravým tlačítkem myši a zvolte **zarážku** > **Odstranit zarážku**) a pak pokračujte stisknutím klávesy **F5** .
...
1. Kliknutím na tlačítko **restartovat** ![aplikaci](../debugger/media/dbg-tour-restart.png "RestartApp") na panelu nástrojů ladění (**CTRL** + **SHIFT** + **F5**) restartujte.

    Ladicí program se pozastaví na řádku kódu, kde jste nastavili zarážku.

1. Najeďte myší na proměnnou `type`. Zobrazí se hodnota `S` (za kódem znaku). Máte zájem o hodnotu `I`, protože víte, že se jedná o nepravidelný typ Galaxy.

1. Stiskněte klávesu **F5** a najeďte na `type` proměnnou znovu. Tento krok opakujte, dokud se ve `type` proměnné nezobrazí hodnota `I`.

    ![Kontrola proměnné](../debugger/media/beginners-inspecting-data.png)

1. Nyní stiskněte klávesu **F11** (**ladění** > **Krok dovnitř** nebo **Krok do** tlačítka na panelu nástrojů ladění).
...
1. Stiskněte klávesu **F11** , dokud nezastavíte řádek kódu v příkazu `switch` pro hodnotu I. Tady vidíte jasný problém, který je výsledkem překlepu. Očekávali jsme, že kód se bude nacházet `MyGType` jako negalaxy typ, ale ladicí program tento kód kompletně přeskočí a pozastaví se v `default` oddílu příkazu `switch`.

    ![Najít překlep](../debugger/media/beginners-typo.png)

    Při prohlížení kódu se v příkazu `case 'l'` zobrazí překlep. Mělo by to být `case 'I'`.
