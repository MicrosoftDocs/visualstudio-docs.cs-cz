...
Chcete-li vložit nové záznamy do databáze, můžete použít metodu `TableAdapter.Update` nebo jednu z DBDirect metod TableAdapter (konkrétně metoda `TableAdapter.Insert`). Další informace najdete v tématu [TableAdapter](../data-tools/create-and-configure-tableadapters.md).
...
Následující postup ukazuje použití metody `TableAdapter.Insert` pro vložení řádků. Tento příklad vloží data do tabulky `Region` v databázi Northwind.
