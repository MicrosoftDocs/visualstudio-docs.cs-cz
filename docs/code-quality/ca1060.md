...
Metody vyvolání platformy, například ty, které jsou označeny pomocí <xref:System.Runtime.InteropServices.DllImportAttribute?displayProperty=fullName> atributu nebo metody, které jsou definovány pomocí `Declare` klíčového slova v [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] , přistupuje k nespravovanému kódu. Tyto metody by měly být v jedné z následujících tříd:
