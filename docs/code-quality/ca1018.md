...
Atribut <xref:System.AttributeUsageAttribute?displayProperty=fullName> není přítomen u vlastního atributu.
...
Výčet <xref:System.AttributeTargets?displayProperty=fullName> definuje cíle, které lze zadat pro vlastní atribut. Pokud <xref:System.AttributeUsageAttribute>vynecháte, bude váš vlastní atribut platný pro všechny cíle, jak je definováno `All` hodnotou výčtu <xref:System.AttributeTargets>.
