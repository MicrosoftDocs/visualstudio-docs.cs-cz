...
Chcete-li opravit porušení tohoto pravidla, implementujte vlastnost rozhraní explicitně (deklarujte ji jako `IEnumerator.Current`). Přidejte veřejnou verzi s silným typem vlastnosti deklarovanou jako `Current`a vraťte objekt silného typu.
...
Následující příklad ukazuje správný způsob implementace typu <xref:System.Collections.IEnumerator> silně typovaného typu.
