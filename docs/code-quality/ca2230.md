...
V C#rozhraní se konvence volání `VarArgs` používá v případě, že seznam parametrů metody končí klíčovým slovem `__arglist`. Visual Basic nepodporuje konvenci volání `VarArgs` a vizuál C++ umožňuje jeho použití pouze v nespravovaném kódu, který používá notaci elipsy `...`.
