---
title: C28112
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
- C28112
helpviewer_keywords:
- C28112
ms.assetid: 2720a5dc-84e9-4f78-a8c7-a320c9f9216b
author: mikeblome
ms.author: mblome
manager: markl
ms.workload:
- multiple
ms.openlocfilehash: f22c46950c0d4f58c97cf75bc5e2cb3c8f91775a
ms.sourcegitcommit: 535ef05b1e553f0fc66082cd2e0998817eb2a56a
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 10/07/2019
ms.locfileid: "72010596"
---
# <a name="c28112"></a>C28112

Upozornění C28112: K proměnné, ke které se dá přistupovat přes propojenou funkci, se musí vždycky přistupovat prostřednictvím propojené funkce.

Viz řádek *[číslo]* : Přístup k proměnné, ke které se přistupuje přes propojenou rodinu funkcí @ no__t-0 jakýmkoli jiným způsobem, není vždy bezpečný.

Proměnnou, ke které se přistoupí pomocí rutiny podpory s podporou spolupracovníka, jako je například InterlockedCompareExchangeAcquire, je později k dispozici pomocí jiné funkce. I když určité běžné přiřazení, přístup a porovnání k proměnným, které jsou používány propojenými rutinami @ no__t-0, lze bezpečně získat pomocí jiné funkce, riziko je dostatečně Skvělé, aby bylo možné prošetřit jednotlivé instance.

## <a name="example"></a>Příklad

Následující příklad kódu vygeneruje toto upozornění:

```cpp
inter_var --;
...
InterlockedIncrement(&inter_var);
```

Následující příklad kódu předchází tomuto upozornění:

```cpp
InterlockedDecrement(&inter_var);
...
InterlockedIncrement(&inter_var);
```
