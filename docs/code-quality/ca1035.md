...
Toto pravidlo vyžaduje <xref:System.Collections.ICollection> implementace, aby poskytovaly členy se silnými typy, aby uživatelé nemuseli přetypování argumentů na typ <xref:System.Object> při použití funkcí poskytovaných rozhraním. Toto pravidlo předpokládá, že typ, který implementuje <xref:System.Collections.ICollection>, tak, aby spravoval kolekci instancí typu, který je silnější než <xref:System.Object>.
...
Chcete-li opravit porušení tohoto pravidla, implementujte člen rozhraní explicitně (deklarujte ho jako <xref:System.Collections.ICollection.CopyTo%2A>). Přidejte veřejný člen se silnými typy deklarovaný jako `CopyTo`a pokaždé, když jako svůj první parametr převezme pole silného typu.
