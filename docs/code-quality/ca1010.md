...
Následující příklad opravuje porušení změnou základní třídy kolekce z neobecné `CollectionBase` třídy na obecnou `Collection<T>` (`Collection(Of T)` v Visual Basic) třídy.
