...
Chcete-li opravit porušení tohoto pravidla, použijte přetížení, které přebírá @no__t argument-0. Nebo použijte [ C# interpolované řetězce](/dotnet/csharp/tutorials/string-interpolation) a předejte je metodě <xref:System.FormattableString.Invariant%2A?displayProperty=nameWithType>.
...
V následujícím kódu řetězec `example1` porušuje pravidlo CA1305. Řetězec `example2` splňuje CA1305 pravidla předáním <xref:System.Globalization.CultureInfo.CurrentCulture%2A?displayProperty=nameWithType>, který implementuje <xref:System.IFormatProvider> na <xref:System.String.Format(System.IFormatProvider,System.String,System.Object)?displayProperty=nameWithType>. Řetězec `example3` splňuje CA1305 pravidla předáním interpolované řetězce do <xref:System.FormattableString.Invariant%2A?displayProperty=fullName]>.
