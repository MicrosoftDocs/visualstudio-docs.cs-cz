...
Když jsou všechna statická data inicializována vložená a explicitní statický konstruktor není deklarován, kompilátory jazyka MSIL (Microsoft Intermediate Language) přidají příznak `beforefieldinit` a implicitní statický konstruktor, který inicializuje statická data, do definice typu jazyka MSIL. Když kompilátor JIT nalezne příznak `beforefieldinit`, většinou nejsou přidány žádné kontroly statického konstruktoru. Statická inicializace je zaručena v určitou dobu před tím, než dojde ke statickým polím, ale ne před vyvoláním statické metody nebo konstruktoru instance. Všimněte si, že statická inicializace může nastat kdykoli po deklarování proměnné typu.
...
Všimněte si Přidání příznaku `beforefieldinit` v definici jazyka MSIL pro třídu `NoStaticConstructor`.
