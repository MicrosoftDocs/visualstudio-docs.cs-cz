...
Následující příklad ukazuje účinek porušení tohoto pravidla. Testovací aplikace vytvoří instanci `DerivedType`, což způsobí spuštění konstruktoru základní třídy (`BadlyConstructedType`). konstruktor `BadlyConstructedType`nesprávně volá virtuální metodu `DoSomething`. Jak výstup ukazuje, `DerivedType.DoSomething()` provede před spuštěním konstruktoru `DerivedType`.
