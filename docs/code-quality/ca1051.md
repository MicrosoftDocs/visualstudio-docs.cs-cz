...
Chcete-li opravit porušení tohoto pravidla, zpřístupněte pole `private` nebo `internal` a zpřístupněte ho pomocí externě viditelné vlastnosti.
...
- Když cílová platforma používá `ref` k úpravě polí, jako jsou například model-View-ViewModel (MVVM) architektury pro WPF a UWP
