...
Vnořené příkazy `using` (`Using` v Visual Basic) můžou způsobit porušení upozornění CA2202. Pokud prostředek IDisposable vnořeného vnitřního `using` příkazu obsahuje prostředek vnějšího příkazu `using`, metoda `Dispose` vnořeného prostředku uvolní obsažený prostředek. Pokud dojde k této situaci, metoda `Dispose` vnějšího příkazu `using` se pokusí o vyřazení svého prostředku podruhé.

V následujícím příkladu je objekt <xref:System.IO.Stream>, který je vytvořen v vnějším příkazu Using, vydán na konci vnitřního příkazu Using v metodě Dispose objektu <xref:System.IO.StreamWriter>, který obsahuje objekt `stream`. Na konci vnějšího příkazu `using` je objekt `stream` vydaný podruhé. Druhá verze je porušení CA2202.
...
Chcete-li tento problém vyřešit, použijte místo vnějšího příkazu `using` `try`/`finally` blok. V bloku `finally` Ujistěte se, že prostředek `stream` nemá hodnotu null.
