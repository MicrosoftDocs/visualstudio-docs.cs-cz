...
|Příznak | Příznaky|<xref:System.Enum?displayProperty=fullName>|
...
Pokud toto pravidlo spouštíte z [analyzátorů FxCop](install-fxcop-analyzers.md) (a ne pomocí starší verze analýzy), můžete nakonfigurovat seznam povolených přípon s každou příponou oddělenou znakem svislé čáry (|). Například chcete-li určit, že pravidlo by nemělo být spuštěno proti příznakům a příponám příznaků, přidejte do souboru. editorconfig v projektu následující dvojici klíč-hodnota:
