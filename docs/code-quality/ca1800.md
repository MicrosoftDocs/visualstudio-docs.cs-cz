...
Následující příklad ukazuje metodu, která porušuje pravidlo pomocí operátoru C# `is`. Druhá metoda splňuje pravidlo tím, že nahrazuje operátor `is` s testem v porovnání s výsledkem operátoru `as`, který snižuje počet operací přetypování na jednu iteraci od dvou k jedné. Třetí metoda také splňuje pravidlo pomocí `is` s [porovnáváním vzorů](/dotnet/csharp/language-reference/keywords/is#pattern-matching-with-is) pro vytvoření proměnné požadovaného typu, pokud by převod typu proběhl úspěšně.
