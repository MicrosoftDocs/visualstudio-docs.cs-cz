...
|>=|>=|>=|CompareTo nebo Compare|
|++|NEUŽÍVÁ SE.|++|Přírůstek|
|!=|<>|!=|Je rovno|
|<<|<<|<<|LeftShift|
|<<=|<<=|<<=|LeftShift|
|<|<|<|CompareTo nebo Compare|
|<=|<=|\<=|CompareTo nebo Compare|
|&&|NEUŽÍVÁ SE.|&&|LogicalAnd|
|&#124;&#124;|NEUŽÍVÁ SE.|&#124;&#124;|Logický operátor|
|!|NEUŽÍVÁ SE.|!|LogicalNot|
...
|~|Not|~|OnesComplement|
|>>|>>|>>|RightShift|
=|NEUŽÍVÁ SE.|>>=|RightShift|
|-(binární)|-(binární)|-(binární)|Odebrat|
|-=|NEUŽÍVÁ SE.|-=|Odebrat|
|true|IsTrue|NEUŽÍVÁ SE.|True (vlastnost)|
|– (Unární)|NEUŽÍVÁ SE.|-|Negate|
|+ (Unární)|NEUŽÍVÁ SE.|+|Plus|
|false|IsFalse|Nepravda|True (vlastnost)|
