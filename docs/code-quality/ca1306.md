...
Národní prostředí Určuje prvky prezentace specifické pro jazykovou verzi pro data, jako je například formátování používané pro číselné hodnoty, symboly měny a pořadí řazení. Při vytváření <xref:System.Data.DataTable> nebo <xref:System.Data.DataSet>byste měli nastavit národní prostředí explicitně. Ve výchozím nastavení je národní prostředí pro tyto typy aktuální jazyková verze. Pro data, která jsou uložena v databázi nebo souboru a jsou sdíleny globálně, by mělo být národní prostředí obvykle nastaveno na invariantní jazykovou verzi (<xref:System.Globalization.CultureInfo.InvariantCulture%2A?displayProperty=fullName>). Když jsou data sdílena mezi kulturami, použití výchozího národního prostředí může způsobit, že obsah <xref:System.Data.DataTable> nebo <xref:System.Data.DataSet> je zobrazován nebo interpretován nesprávně.
