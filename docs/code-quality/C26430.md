...
**C++ Základní pokyny**: F. 23: Použijte Not_Null @ no__t-0T > k označení, že "null" není platná hodnota.

Pokud kód někdy kontroluje hodnotu null proměnných ukazatelů, mělo by to konzistentně provést a ověřit ukazatele na všech cestách. Někdy je deagresivní kontrola hodnoty null stále lepší než možnost závažného selhání v jedné ze složitých větví. V ideálním případě by se měl tento kód Refaktorovat tak, aby byl méně složitý (rozdělením do více funkcí) a spoléhat na značky, jako je GSL:: Not_Null (viz pokyny pro podporu knihovny) a izolují části algoritmu, které mohou zajistit bezpečný předpoklad platných hodnot ukazatelů. Pravidlo TEST_ON_ALL_PATHS pomáhá najít místa, kde jsou kontroly hodnoty null buď nekonzistentní (proto mohou předpoklady vyžadovat kontrolu), nebo skutečné chyby, kde potenciální hodnota null může obejít kontrolu hodnoty null v některých cestách kódu.
