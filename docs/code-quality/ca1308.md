...
Změňte operace, které převádějí řetězce na malá písmena, aby byly řetězce převedeny na velká písmena. Změňte například `String.ToLower(CultureInfo.InvariantCulture)` na `String.ToUpper(CultureInfo.InvariantCulture)`.
