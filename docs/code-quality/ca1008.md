...
Pokud výčet, který má použit <xref:System.FlagsAttribute> definuje člena s nulovou hodnotou, jeho název by měl být None, aby označoval, že ve výčtu nebyly nastaveny žádné hodnoty. Použití členu s nulovou hodnotou pro jakýkoliv jiný účel je v rozporu s použitím <xref:System.FlagsAttribute> v tom, že bitové operátory AND a OR jsou pro člena nepoužitelné. To znamená, že hodnota nula by měla být přiřazena pouze jednomu členu. Pokud se v výčtu příznaků, které mají hodnotu nula, vyskytuje více členů, `Enum.ToString()` vrátí nesprávné výsledky pro členy, které nejsou nulové.
...
Následující příklad ukazuje dva výčty, které splňují pravidlo a výčet, `BadTraceOptions`, které porušují pravidlo.
