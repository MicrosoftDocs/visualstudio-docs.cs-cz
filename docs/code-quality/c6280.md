...
Warning C6280: > \<variable se přiřazuje pomocí \<function >, ale odstraňují se \<function >
 ...
 Toto upozornění by například bylo vytvořeno, pokud je paměť přidělena pomocí `malloc`, ale uvolněna pomocí `GlobalFree` nebo `delete`. Ve zvláštních případech neshody mezi polem `new[]` a skalárním `delete` jsou namísto této zprávy hlášeny přesnější upozornění.
