...
Metoda <xref:System.IDisposable.Dispose%2A?displayProperty=fullName> by neměla vyvolat výjimku. Dispose se často volá jako součást logiky čištění v klauzuli `finally`. Proto explicitní vyvolání výjimky z Dispose vynutí uživatele přidat zpracování výjimek v rámci klauzule `finally`.
