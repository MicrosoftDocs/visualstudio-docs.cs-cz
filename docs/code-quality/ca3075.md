...
> [!NOTE]
> I když <xref:System.Xml.XmlReader.Create%2A?displayProperty=nameWithType> je doporučeným způsobem vytvoření instance <xref:System.Xml.XmlReader>, existují rozdíly v chování <xref:System.Xml.XmlTextReader>. @No__t_0 z <xref:System.Xml.XmlReader.Create%2A> normalizuje `\r\n` na `\n` hodnot XML, zatímco <xref:System.Xml.XmlTextReader> zachová `\r\n` sekvenci.
