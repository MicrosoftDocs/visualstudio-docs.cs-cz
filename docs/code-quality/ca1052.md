...
Pravidlo CA1052 předpokládá, že typ, který obsahuje pouze statické členy, není navržen tak, aby byl zděděn, protože typ neposkytuje žádné funkce, které mohou být přepsány v odvozeném typu. Typ, který není určen k dědění, by měl být označen modifikátorem `static` v C# nástroji, aby bylo možné jeho použití jako základní typ zakázat. Kromě toho by měl být odebrán jeho výchozí konstruktor. V Visual Basic musí být třída převedena na [modul](/dotnet/visual-basic/language-reference/statements/module-statement).
...
Potlačí upozornění z tohoto pravidla pouze v případě, že je typ navržen tak, aby byl zděděn. Absence modifikátoru `static` navrhuje, že typ je užitečný jako základní typ.
