...
Pro toto pravidlo musí být <xref:System.Runtime.InteropServices.ComVisibleAttribute> na úrovni sestavení nastavená na `false` a <xref:System.Runtime.InteropServices.ComVisibleAttribute> třídy musí být nastavená na `true`, jak ukazuje následující kód.
...
V případě, že klient modelu COM nevyžaduje přístup k funkcím, které poskytuje metoda `static`, je bezpečné potlačit upozornění od tohoto pravidla.
...
### <a name="description"></a>Popis
Chcete-li opravit porušení v předchozím příkladu, můžete změnit metodu na metodu instance, která ale v této instanci nedává smysl. Lepším řešením je explicitně použít `ComVisible(false)` na metodu, aby bylo jasné, že by nebylo možné z modelu COM vyjasnit jiným vývojářům, že metodu nelze zobrazit.
