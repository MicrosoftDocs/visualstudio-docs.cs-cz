...
Je běžný postup, jak použít kontrolní výrazy k vymáhání platnosti hodnot ukazatelů. Problém s kontrolními výrazy je, že nezveřejňují předpoklady prostřednictvím rozhraní (např. v návratových typech nebo parametrech). Kontrolní výrazy jsou také těžší udržovat a uchovávat synchronizaci s dalšími změnami kódu. Doporučujeme použít GSL:: Not_Null z pokynů knihovny podpory jako značky prostředků, které by nikdy neměly mít hodnotu null. USE_NOTNULL pravidla pomáhá identifikovat místa, která vyloučí kontroly hodnoty null, a proto je možné ji aktualizovat tak, aby používala GSL:: Not_Null.
...
- Logika pravidla vyžaduje kód pro zrušení odkazu na proměnnou ukazatele, aby bylo možné ověřit, zda je kontrolní hodnota (nebo vynucení hodnoty, která není null) odůvodněná. Upozornění bude vygenerováno pouze v případě, že ukazatelé jsou zpětně odkazovány a nikdy testovány na hodnotu null.
  - Aktuální implementace zpracovává jenom jednoduché ukazatele (nebo jejich aliasy) a nedetekuje inteligentní ukazatele, i když se GSL:: Not_Null dá použít i pro inteligentní ukazatele.
