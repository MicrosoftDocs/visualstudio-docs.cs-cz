...
V následujícím příkladu typ `YourType` rozšiřuje <xref:System.Collections.CollectionBase?displayProperty=fullName>, protože by měly být všechny kolekce silného typu. <xref:System.Collections.CollectionBase> poskytuje explicitní implementaci rozhraní <xref:System.Collections.IList> pro vás. Proto je nutné zadat pouze členy silného typu pro <xref:System.Collections.IList> a <xref:System.Collections.ICollection>.
