...
- `ExcelUICommunicator`  Třída – Tato třída implementuje `IExcelUICommunication` rozhraní.

## <a name="thisaddin-class"></a>ThisAddIn – třída
 Většina této třídy je ve skutečnosti generována Visual Studio Tools for Office v `ThisAddIn.Designer.cs` souboru při vytváření projektu doplňku aplikace Excel.
...
## <a name="exceluicommunicator-class"></a>ExcelUICommunicator – třída
 Tato třída implementuje `IExcelUITestCommunication` rozhraní a získá požadované informace o uživatelském rozhraní z modelu excelového objektu. Další informace najdete v tématu [ukázka rozhraní Excel Communicator](../test/sample-excel-communicator-interface.md).
