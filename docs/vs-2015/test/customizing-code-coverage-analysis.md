...
Ve výchozím nastavení nástroj pro pokrytí Visual Studio Code analyzuje všechna sestavení řešení (. exe/. dll), která jsou načtena během testování částí. Doporučujeme zachovat toto výchozí nastavení, protože ve většině případů funguje dobře. Další informace naleznete v tématu [Použití pokrytí kódu k určení, kolik kódu je testováno](../test/using-code-coverage-to-determine-how-much-code-is-being-tested.md).
...
- *Chci zahrnout sestavení, která nejsou součástí řešení.*
  ...
  Chcete-li přizpůsobit chování pokrytí kódu, zkopírujte [ukázku na konci tohoto tématu](#sample) a přidejte ji do svého řešení pomocí přípony souboru. runsettings. Upravte ho podle svých potřeb a potom v nabídce **test** zvolte možnost **nastavení testu**, vybrat soubor s **nastavením testu** . Zbývající část tohoto tématu popisuje tento postup podrobněji.

## <a name="the-runsettings-file"></a>Soubor s příponou .runsettings
 V souboru s příponou .runsettings jsou uvedena upřesňující nastavení pokrytí kódu. Jedná se o konfigurační soubor používaný nástroji pro testování částí. Doporučujeme zkopírovat [ukázku na konci tohoto tématu](#sample) a upravit ji tak, aby vyhovovala vašim potřebám.

- *Co se stalo se souborem. testsettings I použitým v aplikaci Visual Studio 2010?*
...
3. V nabídce **test** zvolte možnost **nastavení testu**, **Vybrat soubor s nastavením testu** a vyberte soubor.

4. Nyní když spustíte **analýzu pokrytí kódu**, bude tento `.runsettings` soubor řídit jeho chování. Nezapomeňte, že musíte spustit pokrytí kódu znovu: předchozí výsledky pokrytí a barevné zvýraznění kódu nejsou při spuštění testů nebo aktualizaci kódu automaticky skryty.

5. Chcete-li vlastní nastavení vypnout a zapnout, zrušte výběr nebo vyberte soubor v nabídce **test**, **nastavení testu** .

   ![Nabídka nastavení testu se souborem vlastního nastavení](../test/media/codecoverage-settingsfile.png "CodeCoverage – settingsFile")

   Další aspekty testování částí lze nakonfigurovat ve stejném souboru s příponou .runsettings. Další informace najdete v tématu [testování částí kódu](../test/unit-test-your-code.md).
...
### <a name="to-customize-runsettings-in-visual-studio-tests"></a>Přizpůsobení souboru s příponou .runsettings v testech sady Visual Studio
 Zvolte možnost **test**, **nastavení testu**, **Vybrat soubor s nastavením testu** a vyberte soubor. runsettings. Soubor se zobrazí v nabídce Nastavení testu a můžete jej vybrat nebo zrušit. I když je vybraný, váš soubor. runsettings se použije vždy, když použijete možnost **Analyzovat pokrytí kódu**.

### <a name="to-customize-run-settings-in-a-command-line-test"></a>Přizpůsobení parametrů spuštění v testu příkazového řádku
 Pro spuštění testů z příkazového řádku se používá příkaz vstest.console.exe. Soubor nastavení je parametr tohoto nástroje. Další informace naleznete v tématu [použití VSTest. Console z příkazového řádku](https://msdn.microsoft.com/library/852812d8-b3bb-407e-bc43-04d511fcb27a).
...
2. V Team Explorer otevřete **sestavení**a pak přidejte nebo upravte definici sestavení.

3. Na stránce **proces** rozbalte položku **automatizované testy**, **zdroj testu**, **parametry spuštění**. Vyberte soubor **. runsettings** .

   - <em>Místo **zdroje testu**se ale zobrazí **testovací sestavení</em>* . Při pokusu o nastavení pole pro **nastavení běhu** mohu vybrat pouze soubory. testsettings.*

      V části **automatizované testy**vyberte **testovací sestavení**a zvolte **[...]** na konci řádku. V dialogovém okně **Přidat/upravit testovací běh** nastavte **Test Runner** na **Visual Studio Test Runner**.
...
## <a name="see-also"></a>Viz také
 [Použití pokrytí kódu k určení, kolik kódu je testováno](../test/using-code-coverage-to-determine-how-much-code-is-being-tested.md) [testování částí kódu](../test/unit-test-your-code.md)
