...
K sestavování nativních aplikací pro iOS, Android a Windows ze společného základního kódu C#/.NET pomocí Xamarin budete potřebovat následující:  
...
## <a name="pre-requisites"></a><a name="prereq"></a> Předpoklady  
...
1. Pro cílení na iOS:  
    ...
    1. Při používání sady Visual Studio na počítači s Windows (7) jako primárním vývojovým prostředím je síť Mac nutná jenom ke kompilaci a ladění aplikací pro iOS, připojení k simulátoru iOS nebo k připojeným zařízením a k použití návrháře scénářů v aplikaci Visual Studio pro návrh uživatelského rozhraní. Starší modely Mac jsou pro tuto sekundární roli naprosto dostatečné.  
  
## <a name="windows-setup-visual-studio-and-xamarin"></a><a name="windows"></a> Instalační program systému Windows (Visual Studio a Xamarin)  
...
3. Zaškrtněte následující políčka:  
   ...
   1. **Vývoj mobilních aplikací pro různé platformy > C#/.NET (Xamarin)**. V rámci běžných nástrojů a sad pro vývoj softwaru se také automaticky vyberou různé nástroje pro Android. Tato možnost by také měla aktualizovat existující instalaci Xamarin.  
        ...
        ![Výběr možnosti Xamarin v oblasti vývoj mobilních aplikací pro různé&#45;platformy](../cross-platform/media/cross-plat-xamarin-setup-2.png "Instalace na více než 1-platnou instalaci Xamarin 2")  
...
6. Pro Windows 7 a starší nebo běžící Windows na Macu použijte [emulátor Android SDK](https://developer.xamarin.com/guides/android/deployment,_testing,_and_metrics/debug-on-emulator/android-sdk-emulator/) , pokud nemáte fyzická zařízení. Viz poznámka níže.  
   ...
   **Poznámka k emulátorům na počítačích s Windows:** Vzhledem k tomu, že procesory podporují pouze jednu technologii virtualizace, je vhodné používat na vývojovém počítači pouze jeden. Existují tři hlavní technologie virtualizace: Hyper-V (používá emulátor sady Visual Studio pro Android a emulátor Windows Phone), Virtual Box (používaný Genymotion) a Intel modul HAXM (používaný emulátorem Android SDK). Vzhledem k různým problémům mezi technologiemi Hyper-V a virtuálním polem je vhodné použít emulátory pouze jednoho typu v daném počítači, a to proto, že výše uvedená doporučení používají technologii Hyper-V v počítačích s Windows 8 a novějším a emulátory Intel modul HAXM ve Windows 7 a starších verzích a také při spuštění systému Windows na Macu.  
  
## <a name="mac-setup-apple-id-xcode-and-xamarin"></a><a name="mac"></a> Instalace Mac (Apple ID, Xcode a Xamarin)  
  
1. Pokud ho ještě nemáte, vytvořte si bezplatné Apple ID [https://appleid.apple.com](https://appleid.apple.com/) . To je nezbytné pro instalaci a přihlášení do Xcode.  
  
2. Stáhněte si a nainstalujte Xcode z a  [https://developer.apple.com/xcode/](https://developer.apple.com/xcode/) přidejte své Apple ID, jak je popsáno v tématu [Přidání účtu do Xcode](https://developer.apple.com/library/content/documentation/IDEs/Conceptual/AppStoreDistributionTutorial/AddingYourAccounttoXcode/AddingYourAccounttoXcode.html#//apple_ref/doc/uid/TP40013839-CH40-SW1) (Apple.com).  
