...
Následující kód vygeneruje toto upozornění voláním funkce s poznámkou funkce [ReadFile](/windows/win32/api/fileapi/nf-fileapi-readfile) dvakrát. Po prvním volání vlastnost post atributu označí druhý parametr hodnota nedůvěryhodná. Proto předáním nedůvěryhodné hodnoty do druhého volání `ReadFile` vygeneruje toto upozornění, jak je znázorněno v následujícím kódu:  
