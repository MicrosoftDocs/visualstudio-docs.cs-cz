---
title: C6386 | Microsoft Docs
ms.date: 11/15/2016
ms.prod: visual-studio-dev14
ms.technology: vs-ide-code-analysis
ms.topic: reference
f1_keywords:
- C6386
helpviewer_keywords:
- C6386
ms.assetid: 84e69fe8-8f03-4bb3-b194-e5551882e214
caps.latest.revision: 12
author: corob-msft
ms.author: corob
manager: jillfra
ms.openlocfilehash: c77c9c2479d43c1a323c4a247d098deb44790436
ms.sourcegitcommit: 6cfffa72af599a9d667249caaaa411bb28ea69fd
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 09/02/2020
ms.locfileid: "77278633"
---
# <a name="c6386"></a>C6386
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

Upozornění C6386: přetečení vyrovnávací paměti: přístup k \<buffer name> zapisovatelné velikosti je \<size1> bajtů, ale \<size2> mohou být zapsány bajty: řádky: x, y  
  
 Toto upozornění indikuje, že rozsah s možností zápisu zadané vyrovnávací paměti může být menší než index použitý k zápisu. To může způsobit přetečení vyrovnávací paměti.  
  
## <a name="example"></a>Příklad  
 Následující kód generuje toto upozornění i [C6201](../code-quality/c6201.md):  
  
```  
#define MAX 25  
  
void f ( )  
{  
  char ar[MAX];  
  //Code ...  
  ar[MAX] = '\0';  
}  
```  
  
 Chcete-li opravit obě upozornění, použijte následující kód:  
  
```  
#define MAX 25  
  
void f ( )  
{  
   char a[MAX];  
   // code...  
   a[MAX - 1] = '\0';  
}   
```  
  
## <a name="see-also"></a>Viz také  
 [C6201](../code-quality/c6201.md)
