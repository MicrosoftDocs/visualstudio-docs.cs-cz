---
title: C28113 | Microsoft Docs
ms.date: 11/15/2016
ms.prod: visual-studio-dev14
ms.technology: vs-ide-code-analysis
ms.topic: reference
f1_keywords:
- C28113
helpviewer_keywords:
- C28113
ms.assetid: c3f4f143-6985-4274-a87a-59c21a82d67a
caps.latest.revision: 6
author: corob-msft
ms.author: corob
manager: jillfra
ms.openlocfilehash: ad4b311a0e3537656e4629c35e14512ed8e90707
ms.sourcegitcommit: 6cfffa72af599a9d667249caaaa411bb28ea69fd
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 09/02/2020
ms.locfileid: "77265723"
---
# <a name="c28113"></a>C28113
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

Upozornění C28113: přístup k místní proměnné prostřednictvím propojené funkce  
  
 Ovladač používá pro přístup k místní proměnné rutinu s podporou výkonného ředitele, například [InterlockedDecrement](https://msdn.microsoft.com/library/windows/hardware/ff547871.aspx).  
  
 Přestože jsou ovladače povoleny k předání adresy místní proměnné do jiné funkce a poté použití propojené funkce k provozu na této proměnné, je důležité ověřit, že zásobník nebude nepoužit na neočekávaně disku a že proměnná má správný životní čas ve všech vláknech, které ji mohou použít.  
  
## <a name="example"></a>Příklad  
 V následných výpočtech se obvykle místo vstupních argumentů používá návratová hodnota rutiny podpory s propojenými pracovníky. Propojené rutiny jsou také chráněny pouze prvním argumentem (úplně vlevo). Použití propojené rutiny následujícím způsobem nechrání hodnotu globální a často neobsluhuje žádné účely.  
  
```  
InterlockedExchange(&local, global)  
```  
  
 Následující formulář má stejný účinek na data a bezpečně přistupuje k globální proměnné.  
  
```  
local = InterllockedExchange(&global, global)  
  
```
