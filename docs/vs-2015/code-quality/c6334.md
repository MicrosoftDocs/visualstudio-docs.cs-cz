...
Warning C6334: operátor sizeof aplikovaný na výraz s operátorem může vracet neočekávané výsledky.  
 ...
 Toto upozornění indikuje zneužití `sizeof` operátoru. `sizeof`Operátor, při použití na výraz, má za následek velikost typu výsledného výrazu.  
...
```  
 ...
 `x` bude přiřazena hodnota 4, ne 9, protože výsledný výraz již není ukazatelem na pole `a` , ale pouze ukazatel.  
