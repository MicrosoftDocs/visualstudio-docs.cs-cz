...
Další informace o mnoha vylepšeních a nových funkcích v sadě Visual Studio ALM naleznete v tématu [co je nového pro TFS 2015](/tfs/server/whats-new?view=vsts#tfs-2015-rtm).
...
## <a name="sign-in-across-multiple-accounts"></a>Přihlásit se přes několik účtů
 V rámci sady Visual Studio 2015 je nové prostředí s možností snadného přihlašování výrazně jednodušší než váš přístup k online prostředkům, a to i v případě, že máte několik účtů sady Visual Studio. Po přihlášení k aplikaci Visual Studio se automaticky přihlásíte ke všem instancím sady Visual Studio 2015 a Blend na svém počítači. Při přihlášení se automaticky spustí roaming nastavení. V aplikaci Visual Studio 2015 se Váš účet sdílí napříč funkcemi, takže pokud máte dobrý token, můžete k účtům Visual Studio Team Services přistupovat z **Team Explorer**a prostředky a weby z Microsoft Azure předplatného na serveru. Prohlížeč. Prostředky Azure se zobrazí také v dialogovém okně Nový projekt pro Application Insights projekty a v nové části Přidat připojenou službu se zobrazí vaše účty Azure Mobile, Azure Storage, [systém Microsoft Office 365](https://msdn.microsoft.com/office/aa905340.aspx) a [Saleforce.com pro vývojáře](https://developer.salesforce.com/) .dialog.
...
#### <a name="the-net-framework"></a>.NET Framework
 ...
 Další informace najdete v tématu [co je nového](https://msdn.microsoft.com/library/1d971dd7-10fc-4692-8dac-30ca308fc0fa) a na [blogu .NET](http://blogs.msdn.com/b/dotnet/).

#### <a name="c"></a>C++
 Vizuál C++ nabízí významné výhody pro vývoj v jazyce c++ 11/14, podporu pro vývoj mobilních zařízení pro různé platformy, podporu pro obnovitelné funkce a možnost await (aktuálně plánované pro standardizaci v c++ 17), vylepšení a opravy chyb v nástroji. implementace knihovny prostředí runtime jazyka C (CRT C++ ) a standardní knihovna (STL), dialogová okna s možností aktualizace velikosti v knihovně MFC, nové optimalizace kompilátoru, lepší výkon při sestavování, nové diagnostické funkce a nové nástroje pro zvýšení produktivity v editoru kódu.

 Další informace najdete v tématu [co je nového pro vizuál C++ ](https://msdn.microsoft.com/library/1cc09fad-85a2-43c2-b022-bb99f5fe0ad7) a [blog vizuálů C++ ](http://blogs.msdn.com/b/vcblog/).
...
## <a name="cross-platform-debugging-support"></a>Podpora ladění pro různé platformy
 Pomocí sady Visual Studio můžete vytvářet a ladit nativní mobilní aplikace, které běží na zařízeních s Windows, iOS a Androidem. Použijte [emulátor sady Visual Studio pro Android](http://blogs.msdn.com/b/visualstudioalm/archive/2014/11/12/introducing-visual-studio-s-emulator-for-android.aspx)nebo připojte zařízení a ladit kód přímo v aplikaci Visual Studio.
...
- **C# /Xamarin**. Použijte [Xamarin](https://msdn.microsoft.com/library/dn879698\(v=vs.140\).aspx) k vytváření nativních aplikací pro Windows, iOS a Android v systému Visual Studio C#s.
     ...
     [Ladění (iOS](http://developer.xamarin.com/guides/ios/deployment,_testing,_and_metrics/debugging_in_xamarin_ios/) ) a [ladění na zařízení](http://developer.xamarin.com/guides/android/deployment,_testing,_and_metrics/debugging_with_xamarin_android/) v [Průvodci pro vývojáře Xamarin](http://developer.xamarin.com/guides) popisují prostředí ladění.
...
### <a name="perftips"></a>Tipy pro výkon
 Tipy pro výkon zobrazí dobu provádění metod během ladění, což vám umožní rychle odhalit slabá místa, aniž by bylo nutné vyvolávat Profiler. Chcete-li začít, přečtěte si téma [tipy pro výkon: informace o výkonu při ladění se sadou Visual Studio.](http://blogs.msdn.com/b/visualstudioalm/archive/2014/08/18/perftips-performance-information-at-a-glance-while-debugging-with-visual-studio.aspx)
...
### <a name="gpu-usage-tool"></a>Nástroj využití GPU
 Nástroj využití GPU vám pomůže shromažďovat a analyzovat data o využití GPU v aplikacích a hrách DirectX a řešit potíže s tím, že pocházely z procesoru nebo GPU na výkon. Chcete-li začít s nástrojem, přečtěte [si C++ Blogový příspěvek týmu vizuálu](http://blogs.msdn.com/b/vcblog/archive/2014/09/05/gpu-usage-tool-in-visual-studio-2013-update-4-ctp1.aspx).

## <a name="live-code-analysis-light-bulbs"></a>Živá analýza kódu (žárovky)
 Nový Roslyn kompilátor pro C# a Visual Basic nejen poskytuje rychlejší dobu kompilace – zároveň umožňuje zcela nové scénáře, jako je například analýza živých kódů, která poskytuje bohatou a přizpůsobitelnou zpětnou vazbu a návrhy přímo v editoru kódu. textový. V aplikaci Visual Studio 2015 se žárovky zobrazují na levém okraji (při použití klávesnice) nebo popisu tlačítka (při najetí myší na chybu pomocí myši). Žárovka v reálném čase oznamuje, že kompilátor (možná použití vlastní sady pravidel) zjistil problém ve vašem kódu a obsahuje také návrh, jak tento problém vyřešit. Po zobrazení žárovky klikněte na ni a vyhledejte možné návrhy.
