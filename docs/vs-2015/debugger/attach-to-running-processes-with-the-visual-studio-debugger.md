...
|Vzdálené ladění aplikace klasické pracovní plochy Windows|Remote Tools a F5|NEUŽÍVÁ SE.| Zobrazit [vzdálené ladění](../debugger/remote-debugging.md)|
|Vzdálené ladění Universal Windows (UPW), OneCore, HoloLens a IoT aplikací|Ladit nainstalovaný balíček aplikace|NEUŽÍVÁ SE.|Použití **ladění / jiné cíle ladění / ladění balíčku nainstalované aplikace** místo **připojit k procesu**|
|Ladění aplikace Universal Windows (UPW), OneCore, HoloLens a IoT, které se nepovedlo spustit ze sady Visual Studio|Ladit nainstalovaný balíček aplikace|NEUŽÍVÁ SE.|Použití **ladění / jiné cíle ladění / ladění balíčku nainstalované aplikace** místo **připojit k procesu**|
