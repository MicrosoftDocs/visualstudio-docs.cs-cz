...
## <a name="example-of-fixing-a-css-rule"></a>Příklad opravy pravidla šablony stylů CSS  
 Tento příklad ukazuje, jak zkontrolovat pravidla šablony stylů CSS a jak ladit problémy se stylem. V tomto příkladu řekněme, že chcete změnit barvu písma používaného k zobrazení názvů skupin v [!INCLUDE[win8_appname_long](../includes/win8-appname-long-md.md)] šabloně rozdělená aplikace.  
...
1. V aplikaci Visual Studio vytvořte [!INCLUDE[win8_appname_long](../includes/win8-appname-long-md.md)] aplikaci pomocí JavaScriptu a HTML v šabloně projektu rozdělené aplikace.  
...
3. Nahraďte následující kód šablony stylů CSS:  
    ...
    ```  
     ...
     Přidáte tak styl, který určuje barvu #ff6a00 (oranžová) pro každou položku v seznamu. Selektor šablon stylů CSS `.itemspage .itemslist .item` označuje sadu názvů tříd pro prvky DIV v items.html, které se zobrazí jako vnořené prvky v živém modelu DOM. `item`Element div určuje položky seznamu.  
...
8. Klikněte na vyznačený název. Průzkumník modelu DOM automaticky vybere odpovídající element HTML, který vypadá podobně jako tento.  
    ...
    ```  
     ...
     Vyberete-li v průzkumníku modelu DOM element H4, karty průzkumníku modelu DOM zobrazí pravidla přidružená k elementu H4. Tady se zobrazí **vypočítaná** karta s `color` otevřenou vlastností:  
  
     ![Karta styly trasování v Průzkumníkovi modelu DOM](../debugger/media/js-css-styles.png "JS_CSS_Styles")  
  
     Toto zobrazení poskytuje užitečné informace o pravidlech, která jsou asociována s tímto `color` stylem, jako jsou následující:  
  
    - Selektor šablon stylů CSS, který jsme změnili v Items. CSS, se `.itemspage .itemslist .item` nepoužívá v konečném výpočtu stylu (zobrazuje se v přeškrtnutí textu). Nepoužívá se ani několik dalších výskytů `color` stylu.  
  
        > [!TIP]
        > U delších názvů selektoru se plný název zobrazí v popisu tlačítka.  
  
    - Finální vypočtená hodnota CSS `rgba(255, 255, 255, 0.87)` je nastavena konkrétně pro následující Selektor šablon stylů CSS: `.itemspage .itemslist .item .item-overlay .item-title` , který je také definován v Items. CSS.  
...
9. Zrušte zaškrtnutí políčka pro první výskyt `color` stylu, který je pro `.itemspage .itemslist .item .item-overlay .item-title` selektor. Nyní se v simulátoru zobrazí, že barva názvů položek se všechny změnila na oranžovou, jak jsme zamýšleli, a selektor, který jsme změnili v CSS, už `.itemspage .itemslist .item` není přepsaný (to znamená, že už se nepoužívá přeškrtnutí textu). Tady je **vypočítaná** karta po zrušení zaškrtnutí políčka.  
...
10. Vyberte kartu **změny** .  
     ...
     Pomocí karty **změny** můžete identifikovat a sledovat změny stylu, které jste provedli během relace ladění. Následující ilustrace znázorňuje selektor na `.itemspage .itemslist .item .item-overlay .item-title` kartě **změny** , který je nyní přepsán.  
...
13. Otevřete `.itemspage .itemslist .item .item-overlay .item-title` selektor stylů.  
  
14. Vyberte první výskyt `color` stylu a pak dvakrát klikněte na hodnotu vlastnosti `rgb(255, 255, 255, 0.87)` .  
  
15. Tuto hodnotu můžete změnit pomocí klávesnice. Změňte na `rgb(255, 255, 0, 0.87)` a potom stiskněte klávesu ENTER. Barvy všech názvů položek v simulátoru se změní na žlutou.  
  
16. Chcete-li provést změny zdrojového souboru šablon stylů CSS, klikněte na odkaz **Items. CSS** na kartě **styly** . Tím se otevře položka. CSS, kde můžete změnit hodnotu `color` stylu v kódu aplikace. Pokud chcete aplikaci aktualizovat bez zastavení a restartování ladicího programu, klikněte na tlačítko  ![aktualizovat aplikaci pro Windows](../debugger/media/js-refresh.png "JS_Refresh") (**Obnovit aplikaci systému Windows**) na panelu nástrojů **ladění** .  
