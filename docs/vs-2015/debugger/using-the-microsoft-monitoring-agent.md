...
## <a name="SetUpMonitoring"></a>Krok 1: nastavení Microsoft Monitoring Agent  
...
2. [Stáhněte si bezplatný Microsoft Monitoring Agent](https://go.microsoft.com/fwlink/?LinkId=320384), buď 32 **. exe** , nebo 64 verze **MMASetup-AMD64. exe**, z webu Microsoft Download Center na váš webový server.  
...
#### <a name="PowerShell2"></a>Otázka: Co když mám prostředí Windows PowerShell 2,0?  
...
#### <a name="FullPermissionsITLog"></a>Otázka: Návody nastavit oprávnění pro fond aplikací?  
...
## <a name="MonitorEvents"></a>Krok 2: zahájení monitorování aplikace  
 Pomocí příkazu Windows PowerShell [Start-WebApplicationMonitoring](https://go.microsoft.com/fwlink/?LinkID=313686) můžete začít monitorovat svoji aplikaci. Pokud používáte System Center 2012, přečtěte si téma [monitorování webových aplikací pomocí Microsoft Monitoring Agent](https://technet.microsoft.com/library/dn465157.aspx).  
...
2. Spusťte příkaz [Start-WebApplicationMonitoring](https://go.microsoft.com/fwlink/?LinkID=313686) , abyste mohli začít monitorovat vaši aplikaci. Tato akce restartuje všechny webové aplikace na webovém serveru.  
...
3. Pokud chcete zjistit stav všech monitorovaných webových aplikací, spusťte příkaz [Get-WebApplicationMonitoringStatus](https://go.microsoft.com/fwlink/?LinkID=313685) .  
  
### <a name="q--a"></a>Dotazy a odpovědi  
  
#### <a name="Minimizing"></a>Otázka: Návody získat většinu dat bez zpomalení aplikace?  
...
- Zkontrolujte moduly, pro které Agent shromažďuje data v plánu shromažďování dat. Upravte plán shromažďování dat tak, aby obsahoval pouze ty moduly, které vás zajímají.  
   ...
   **A:** Ve výchozím nastavení plány shromažďování dat vyloučí moduly nastavením atributu `isExclusionList` na hodnotu `true`. To však může stále shromažďovat data z modulů, které nesplňují kritéria seznamu nebo které vás nemusejí zajímat, například moduly třetích stran nebo open source.  
...
- Primitivní datové typy v polích pro objekty nejvyšší úrovně předávané a vrácené z metod  
  ...
  Agent zaznamenává hodnoty pro `id`, `Employee.Id`, `Employee.Name` a objekt `Employee` vrácený metodou `AlterEmployee`. Agent však nezaznamenává informace o objektu `Address`, který je jiný než bez ohledu na to, zda byl null nebo ne. Agent také nezaznamenává data týkající se místních proměnných v metodě `AlterEmployee`, pokud jiné metody nepoužívají tyto místní proměnné jako parametry, které ukazují, že jsou zaznamenávány jako parametry metody.  
...
2. Spuštěním příkazu [Stop-WebApplicationMonitoring](https://go.microsoft.com/fwlink/?LinkID=313687) vytvořte protokol IntelliTrace a zastavte monitorování konkrétní webové aplikace:  
