...
Nástroj příkazového řádku VSPerfReport slouží k vytváření sestav pomocí  [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] Nástroje pro profilaci datových souborů profilování. Výchozí formát sestavy je soubor. csv.  
...
```  
 ...
 Nástroj příkazového řádku VSPerfReport se používá také k porovnání souborů. vsp nebo. vsps. Pokud chcete vygenerovat sestavu rozdílů (rozdílové), použijte tuto syntaxi:  
  
```  
VSPerfReport [/U] /diff vspfilename1 vspfilename2 [/options]  
```  
  
 `vspfilename1 and vspfilename2` musí se jednat o platné soubory. vsp nebo. vsps.  
...
|**PackSymbols**|Uloží symboly do souboru dat profilování (. vsp) tak, aby se soubory symbolů (. pdb) nevyžadovaly pro analýzu.|  
...
|Možnosti|Popis|  
|-------------|-----------------|  
|**Diff**  `vspfile1 vspfile2`|Porovnejte dva soubory sestav (. vsp nebo. vsps). Možnosti souhrnu budou ignorovány pomocí možnosti diff.|  
