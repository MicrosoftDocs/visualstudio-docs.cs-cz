...
## <a name="tableadapter-update-method"></a>Metoda TableAdapter Update
 Objekty TableAdapter používají příkazy pro čtení a zápis dat z databáze. Počáteční `Fill` (hlavní) dotaz TableAdapter se používá jako základ pro vytvoření schématu přidružené datové tabulky a `InsertCommand` `UpdateCommand` příkazů, a `DeleteCommand` přidružených k `TableAdapter.Update` metodě. Voláním metody TableAdapter `Update` se spustí příkazy, které byly vytvořeny při původní konfiguraci TableAdapter, nikoli jeden z dalších dotazů, které byly přidány pomocí **Průvodce konfigurací dotazů TableAdapter**.
...
> [!NOTE]
> Pokud v hlavním dotazu není k dispozici dostatek informací, příkazy `InsertCommand`, `UpdateCommand` a `DeleteCommand` jsou vytvořeny jako výchozí při generování objektu TableAdapter. Pokud je hlavní dotaz TableAdapter více než jeden příkaz SELECT tabulky, je možné, že návrhář nebude moci generovat  `InsertCommand` , `UpdateCommand` a `DeleteCommand` . Pokud tyto příkazy nejsou vygenerovány, může při spuštění metody dojít k chybě `TableAdapter.Update` .
