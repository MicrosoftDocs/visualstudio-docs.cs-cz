...
2. Přetáhněte uzel souvisejících **objednávek** z okna **zdroje dat** do formuláře **Form1**.
    ...
    > Uzel souvisejících **objednávek** je umístěný pod sloupcem **Fax** a je podřízeným uzlem uzlu **Customers (zákazníci** ).
     ...
     Na formuláři se zobrazí ovládací prvek <xref:System.Windows.Forms.DataGridView> a pruh nástrojů (<xref:System.Windows.Forms.BindingNavigator>) pro procházení záznamů. OrdersTableAdapter a <xref:System.Windows.Forms.BindingSource> se zobrazí v zásobníku komponent.
...
2. Nahraďte kód v obslužné rutině události pro volání metody `Update` souvisejícího objekty TableAdapter. Následující kód nejprve vytvoří tři dočasné tabulky dat, které uchovávají aktualizované informace pro každý <xref:System.Data.DataRowState> (<xref:System.Data.DataRowState>, <xref:System.Data.DataRowState> a <xref:System.Data.DataRowState>). Pak jsou aktualizace spouštěny ve správném pořadí. Kód by měl vypadat takto:
