...
- [Ověřování JavaScriptu](#Validation)
  ...
  Další informace o funkcích technologie IntelliSense v nástroji [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] naleznete v tématu [Using IntelliSense](../ide/using-intellisense.md).

## <a name="determining-intellisense-context"></a><a name="DeterminingIntelliSenseContext"></a> Určení kontextu IntelliSense
...
- `script` prvky s `src` atributy, které odkazují na jiný soubor skriptu. Cílový soubor skriptu musí mít příponu názvu souboru .js.

- JavaScriptové soubory, které odkazují na jiné soubory JavaScriptu pomocí `reference` direktivy.
...
- <xref:System.Web.UI.ScriptManager> <xref:System.Web.UI.ScriptManagerProxy> Ovládací prvky a, pokud webová aplikace je ASP.NET aplikace s podporou jazyka AJAX.

- V [!INCLUDE[atlaslib_current_ext](../includes/atlaslib-current-ext-md.md)] případě, že pracujete v ASP.NET webové aplikaci s podporou AJAX.

    > [!NOTE]
    > Technologie IntelliSense není podporována pro skript, který je v atributech obslužných rutin událostí v prvcích HTML nebo který je definován v `href` atributech.

## <a name="processing-intellisense-information"></a><a name="ProcessingIntelliSenseInformation"></a> Zpracování informací technologie IntelliSense
...
## <a name="javascript-intellisense-features"></a><a name="Features"></a> JavaScript – funkce technologie IntelliSense
...
### <a name="html-dom-elements"></a><a name="HTMLDom"></a> HTML elementy DOM
 Technologie IntelliSense jazyka JavaScript poskytuje programovací odkazy pro prvky modelu DOM jazyka HTML (DHTML), jako například `body` , `form` a `div` . Technologie IntelliSense zobrazuje pouze prvky, které jsou obsaženy v aktuálním dokumentu a na hlavní stránce. JavaScript IntelliSense také podporuje `window` objekty a `document` a jejich členy.

### <a name="intrinsic-objects"></a><a name="IntrinsicObjects"></a> Vnitřní objekty
...
### <a name="user-defined-variables-functions-and-objects"></a><a name="UserDefined"></a> Uživatelem definované proměnné, funkce a objekty
 Když změníte soubor JavaScriptu, [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] prohledá otevřené a odkazované dokumenty a určí všechny dostupné prostředky kódu. To zahrnuje proměnné, funkce a objekty, které jste vytvořili. Tyto prostředky jsou pak k dispozici technologii IntelliSense jazyka JavaScript.
...
### <a name="external-file-references"></a><a name="External"></a> Odkazy na externí soubory
...
#### <a name="script-references"></a><a name="Script"></a> Odkazy na skripty
 ...
 Pokud nepracujete s webovou stránkou s podporou AJAX ASP.NET, můžete odkazovat na externí soubor skriptu pomocí `src` atributu v otevírací značce `script` elementu. `src`Atribut určuje adresu URL externího souboru, který obsahuje zdrojový kód nebo data.

 Následující příklad ukazuje značky, které používají `src` atribut v <`script` značka> pro odkazování na soubor skriptu.
...
```
 ...
 Pokud pracujete s webovou stránkou s podporou AJAX ASP.NET, můžete odkazovat soubory skriptu pomocí <xref:System.Web.UI.ScriptReference> objektu <xref:System.Web.UI.ScriptManager> ovládacího prvku.

 Následující příklad ukazuje značky, které používají <xref:System.Web.UI.ScriptReference> objekt v <xref:System.Web.UI.ScriptManager> ovládacím prvku pro odkazování na soubor skriptu.
...
#### <a name="reference-directives"></a><a name="ReferenceDirectives"></a> Direktivy Reference
 `reference`Direktiva umožňuje [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] navázat relaci mezi aktuálně upravováným skriptem a jinými skripty. `reference`Direktiva umožňuje zahrnout soubor skriptu do kontextu skriptování aktuálního souboru skriptu. Technologie IntelliSense tak může při kódování odkazovat na externě definované funkce, typy a pole.

 `reference`Direktivu vytvoříte ve formě komentáře XML. Direktiva musí být deklarována v souboru dříve než jakýkoli skript. `reference`Direktiva může zahrnovat odkaz na skript na základě disku, odkaz na skript na základě sestavení, odkaz na skript na základě služby nebo odkaz na skript na základě stránky.
...
> Technologie IntelliSense jazyka JavaScript není podporována pro skript, který je obsažen v souborech webové služby (.asmx) v projektech webových aplikací (WAP).
 ...
 Následující pravidla se vztahují na `reference` direktivu.

- `reference`Komentář XML musí být deklarován před jakýmkoli skriptem.
...
#### <a name="reference-groups"></a><a name="ReferenceGroups"></a> Referenční skupiny
...
- Implicitní (Windows) pro aplikace, které [!INCLUDE[win8_appname_long](../includes/win8-appname-long-md.md)] používají JavaScript. Soubory zahrnuté v této skupině jsou v oboru pro každý soubor .js otevřený v editoru kódu pro projekt určeného typu.
...
### <a name="xml-documentation-comments"></a><a name="XMLDocComments"></a> Dokumentační komentáře XML
...
### <a name="aspnet-ajax-objects"></a><a name="ASPNet"></a> ASP.NET AJAX objekty
...
## <a name="javascript-intellisense-extensibility"></a><a name="Extensibility"></a> Rozšiřitelnost technologie IntelliSense pro JavaScript
...
## <a name="javascript-validation"></a><a name="Validation"></a> Ověřování JavaScriptu
 Ověření skriptování v jazyce JavaScript probíhá neustále na pozadí. Když [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] detekuje chyby syntaxe v kódu JavaScriptu, je poskytnuta zpětná vazba následujícími způsoby:
...
- [Vypsat členy](https://msdn.microsoft.com/1b9cc469-9cd4-4d42-9999-1f9479635ff8)
- [Atribut SRC – &#124; vlastnost src](https://msdn2.microsoft.com/library/ms534642.aspx)
