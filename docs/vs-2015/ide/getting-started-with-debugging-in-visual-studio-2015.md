...
### <a name="configuring-a-build"></a>Konfigurace sestavení
 ...
 Poznámka: při sestavování projektu se použijí také hodnoty konfigurace a platforma k určení, která cesta k adresáři projektu je vytvořena pro uložení spustitelného souboru. Obvykle se jedná o ** \<path-to-project> \\<\> \\ \> \\<platforma \><konfigurace projektu**. Například projekt s konfigurací `Debug` a platformou `x86` by byl nalezen v části `Projects\MyProjectNameHere\MyProjectNameHere\bin\Debug\x86` . To může být užitečné, pokud máte vlastní nástroje nebo skripty, které spravují tyto sestavené spustitelné soubory.
...
### <a name="debugging-your-running-code"></a>Ladění spuštěného kódu
 ...
 Pokud váš kód běžel dokonale a přesně podle očekávání, Blahopřejeme! Změňte konfiguraci buildu na **release** a znovu ji Sestavte pro nasazení. (Odborníci můžou chtít přejít na bit testování částí na konci, ale.) Pokud ale přestane reagovat nebo došlo k chybě nebo vám poskytla nějaké neobvyklé výsledky, budete muset najít zdroje těchto problémů a opravit chyby.
...
1. Pro zúžení zdroje selhání nebo programu, který neodpovídá, je třeba ho v celém a kolem kódu volání metody, o které se domníváte, způsobovat selhání. Při procházení kódu odeberte a pak vynulujte zarážky, dokud nenajdete problematický řádek kódu.
...
4. Pokud píšete kód jazyka C nebo C++, pomocí zarážek zastavte kód, abyste mohli zkontrolovat hodnoty adresy (hledání hodnoty NULL) a počty odkazů při ladění selhání souvisejících s pamětí.
...
### <a name="inspecting-your-code-at-run-time"></a>Kontrola kódu za běhu
 ...
 ![Kontrola hodnot&#45;času běhu sady Visual Studio 2015](../ide/media/vs-ide-gs-debug-inspect-value.PNG "vs_ide_gs_debug_inspect_value")
