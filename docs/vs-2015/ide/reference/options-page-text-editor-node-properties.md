...
Tento dokument popisuje některé stránky (nebo kolekce vlastností), které jsou přidruženy ke kategorii **textový editor** , `DTE.Properties("TextEditor", <Property Page>)` dialogového okna **Možnosti** . Název každého dílčího oddílu je volání, které se používá pro přístup ke kolekci `Properties` a tabulka v každém pododdílu obsahuje seznam vlastností v kolekci.
...
|------------------------|-----------|-----------------|
|GoToAnchorAfterEscape|Get/Set (Boolean)|Pokud je `True`, stisknutí klávesy Escape, když existuje výběr, se přesune kurzor na místo, kde byla spuštěna akce, která vytvořila výběr. `False` přesune kurzor na druhý konec výběru.|
...
## <a name="plain-text"></a>Prostý text
 ...
 Možnosti editoru `PlainText` ovlivňují nastavení editoru při úpravách textových souborů. Každý programovací jazyk a [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] balíček má vlastní specifické nastavení **textového editoru** . Například chcete-li zobrazit nebo změnit nastavení editoru [!INCLUDE[csprcs](../../includes/csprcs-md.md)], použijte `DTE.Properties("TextEditor", "CSharp") or DTE.Properties("TextEditor", "CSharp-Specific")`. Pro nastavení editoru **skriptu SQL** použijte `DTE.Properties("TextEditor", "SQL ")`.
...
|VirtualSpace|Get/Set (Boolean)|Určuje, zda se prázdné znaky zobrazují jako grafika. Když se tato možnost nastaví `true` způsobí, že se položka vlastnosti `WordWrap` (v tomto seznamu) nastaví na `false`.|
|WordWrap|Get/Set (Boolean)|Určuje, zda se dlouhé řádky v tomto zobrazení zalamují na hranicích slov. Když se tato možnost nastaví `true` způsobí, že se položka vlastnosti `VirtualSpace` (v tomto seznamu) nastaví na `false`.|
...
|IndentStyle|Získat nebo nastavit (<xref:EnvDTE.vsIndentStyle>)|Určuje styl odsazení: výchozí, inteligentní nebo žádné.|
|TabSize|Get/Set (Long)|Představuje počet mezer, které se rovnají kartě. nastavení celého čísla mimo rozsah 1 až 60 (včetně) se nezdařila.|
