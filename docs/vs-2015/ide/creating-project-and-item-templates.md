...
## <a name="visual-studio-templates"></a>Šablony aplikace Visual Studio
 Několik předdefinovaných šablon projektů a položek je nainstalováno již po instalaci aplikace [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]. Šablony projektu [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] a [!INCLUDE[csprcs](../includes/csprcs-md.md)] model Windows Forms aplikace a knihovny tříd, které jsou k dispozici v dialogovém okně **Nový projekt** , jsou příklady šablon projektů. Šablony nainstalované položky jsou k dispozici v dialogovém okně **Přidat novou položku** a zahrnují položky jako soubory kódu, soubory XML, stránky HTML a šablony stylů.
