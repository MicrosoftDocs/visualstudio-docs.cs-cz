...
|**Texel rozdíl**|Vypíše rozdíl (vzdálenost) mezi texelů mapy textury 2D.<br /><br /> Rozdíl Texel můžete použít k ukázce sousedních hodnot Texel v shaderu.<br /><br /> **Výkonem**<br /><br /> `Output`: `float2`<br /> Rozdíl (vzdálenost) od Texel k dalšímu Texel (úhlopříčně přesunutý v kladném směru) vyjádřený jako vektor v normalizovaném prostoru textury. Pozice všech sousedních texelů můžete odvodit selektivním ignorováním nebo negací souřadnic U nebo V rozdílu.|**Textura**<br /> Registr textury, který je přidružen k Texel rozdílu.|
