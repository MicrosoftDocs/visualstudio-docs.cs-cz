...
Metadata as source umožňují zobrazit metadata, která se zobrazí jako zdrojový kód C# ve vyrovnávací paměti jen pro čtení. To umožňuje zobrazit deklarace typů a členů (bez implementace). Metadata můžete zobrazit jako zdroj spuštěním příkazu **Přejít k definici** pro typy nebo členy, jejichž zdrojový kód není dostupný z vašeho projektu nebo řešení.
...
## <a name="viewing-metadata-as-source-in-the-code-editor"></a>Zobrazení metadat jako zdroje v editoru kódu
 ...
 Například pokud spustíte příkaz **Přejít na definici** pro <xref:System.Console> , <xref:System.Console> zobrazí se metadata v editoru kódu jako zdrojový kód C#, který se podobá jeho deklaraci, ale bez implementace.
...
## <a name="viewing-metadata-as-source-in-the-code-definition-window"></a>Zobrazení metadat jako zdroje v okně Definice kódu
 ...
 Například pokud umístíte kurzor do slova <xref:System.Console> v editoru kódu, <xref:System.Console> zobrazí se metadata jako zdroj v okně **definice kódu** . Zdroj se podobá <xref:System.Console> deklaraci, ale bez implementace.
