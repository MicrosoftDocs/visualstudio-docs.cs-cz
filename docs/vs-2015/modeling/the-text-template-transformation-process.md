...
|Hostitel|Hostitel je rozhraní mezi modulem a uživatelským prostředím. [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] je hostitelem procesu transformace textu.|Yes. Můžete napsat vlastního hostitele.|
...
- Zpracování chyb transformace šablony textu. Například hostitel může zobrazit chyby v uživatelském rozhraní nebo je zapsat do souboru. (V systému se [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] chyby zobrazují v okně chybové zprávy.)
