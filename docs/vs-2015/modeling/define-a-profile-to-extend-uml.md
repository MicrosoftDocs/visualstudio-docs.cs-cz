...
## <a name="how-to-define-a-profile"></a><a name="DefineProfile"></a> Jak definovat profil
...
3. Přidejte profil do rozšíření sady Visual Studio ( `.vsix` soubor). Můžete buď vytvořit nové rozšíření pro váš profil, nebo přidat profil k existujícímu rozšíření.
...
6. Odeslat soubor s příponou jiným uživatelům, kteří [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] mají nainstalovat na jejich počítače.

## <a name="how-to-add-a-profile-to-a-visual-studio-extension"></a><a name="AddProfile"></a> Postup přidání profilu do rozšíření sady Visual Studio
...
1. Vytvořte projekt rozšíření aplikace Visual Studio.
   ...
   > [!NOTE]
   > [!INCLUDE[vsipsdk](../includes/vsipsdk-md.md)]Abyste mohli tento postup použít, musíte mít nainstalovanou aplikaci.

   1. V nabídce **soubor** přejděte na příkaz **Nový**a klikněte na **projekt**.

   2. V dialogovém okně **Nový projekt** , v části **Nainstalované šablony**, rozbalte položku **Visual C#**, klikněte na **rozšiřitelnost**a potom klikněte na **projekt VSIX**. Nastavte název projektu a klikněte na tlačítko **OK**.
...
8. **Extrakce VSIX pro nasazení**
   ...
   1. V Průzkumníku Windows otevřete složku **.\bin\debug** nebo **.\bin\Release** a vyhledejte soubor **. vsix** . Toto je [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] soubor rozšíření. Může být nainstalováno do vašeho počítače a odesláno jiným uživatelům aplikace Visual Studio.

   2. Instalace rozšíření:

       1. Dvakrát klikněte na `.vsix` soubor. Spustí se instalační program rozšíření sady Visual Studio.
...
2. Upravit `[Content_Types].xml` tak, aby obsahovalo následující text. Všimněte si, že obsahuje záznam pro každou příponu názvu souboru.
...
3. Zkopírujte existující `extension.vsixmanifest` a upravte ho pomocí editoru XML. Změňte uzly ID, název a obsah.

    - Příklad můžete najít `extension.vsixmanifest` v tomto adresáři:
...
5. Přejmenujte soubor zip a změňte příponu názvu souboru z `.zip` na `.vsix` .

6. Pro instalaci profilu na libovolný počítač s odpovídajícími edicemi sady Visual Studio poklikejte na `.vsix` soubor.

#### <a name="to-install-a-uml-profile-from-a-visual-studio-extension"></a>Instalace profilu UML z rozšíření sady Visual Studio

1. Dvakrát klikněte na `.vsix` soubor v Průzkumníku Windows nebo ho otevřete v sadě Visual Studio.
...
## <a name="how-to-define-localized-profiles"></a><a name="Localized"></a> Jak definovat lokalizované profily
...
3. Změňte název nového adresáře na krátký kód ISO pro lokalizovanou jazykovou verzi, například `bg` pro bulharštinu nebo `fr` pro francouzštinu. Měli byste použít neutrální kód kultury, obvykle dvě písmena, nikoli konkrétní jazykovou verzi, jako je například `fr-CA` . Další informace o kódech kultury naleznete v tématu [CultureInfo. GetCultures](https://msdn.microsoft.com/library/system.globalization.cultureinfo.getcultures(VS.100).aspx), který poskytuje úplný seznam kódů jazykové verze.

4. Přidejte do nového adresáře kopii výchozího profilu. Neměňte název souboru.

     Ukázková [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] Složka rozšíření, před sestavením nebo komprimací do `.vsix` souboru, by obsahovala následující složky a soubory:
    ...
    > [!NOTE]
    > Neměli byste vkládat `extension.vsixmanifest` odkazy na lokalizované verze profilů. Zkopírované soubory profilu musí mít stejný název jako profil v nadřazené složce.
...
## <a name="the-structure-of-a-profile"></a><a name="Schema"></a> Struktura profilu
 ...
 Pro usnadnění úprav souborů profilu nainstalujte `.xsd` soubor v:

 **%ProgramFiles%\Microsoft Visual Studio [verze] \Xml\Schemas**

 V této části se jako příklad používá profil C#. Úplnou definici profilu najdete v tématu:
...
> [!NOTE]
> Atribut s názvem nesmí `name` obsahovat mezery ani interpunkční znaménko. Atribut `displayName` , který se zobrazí v uživatelském rozhraní, by měl být platným řetězcem XML.
...
- `<stereotypes>` – definice stereotypu. Každá definice zahrnuje názvy a typy vlastností, které jsou přidány do cílového prvku modelu.

#### <a name="property-types"></a>Typy vlastností
 `<propertyTypes>`Oddíl deklaruje seznam typů, které se používají pro vlastnosti v `<stereotypes>` části. Existují dva druhy typů vlastností: external a Enumeration.
...
#### <a name="metaclasses"></a>Metatřídy
 `<metaclasses>`Oddíl je seznam typů prvků modelu, na které lze definovat stereotypy v tomto profilu:
...
> [!NOTE]
> Název monikeru musí začínat `/yourProfileName/` , kde `yourProfileName` je definováno v `name` atributu profilu ("CSharpProfile" v tomto příkladu). Moniker končí názvem jedné z položek v oddílu metatřídy.

 Každý stereotyp může vypsat nula nebo více vlastností, které přidá do libovolného prvku modelu, na který je použit. `<propertyType>`Obsahuje odkaz na jeden z typů, které jsou definovány v `<propertyTypes>` části. Odkaz musí být buď, aby `<externalTypeMoniker>` odkazoval na nebo, `<externalType>,` `<enumerationTypeMoniker>` aby odkazoval na `<enumerationType>` . Tento odkaz znovu začíná názvem vašeho profilu.
...
## <a name="model-element-types"></a><a name="Elements"></a> Typy prvků modelu
...
3. Zajistěte, aby byla syntaxe popsána zde, a jak je znázorněno na _jednotce_**: \Program Files\Microsoft Visual \\ Studio [Version] \Common7\IDE\Extensions\Microsoft\Architecture Tools\UmlProfiles**.

4. Odinstalujte poškozenou příponu. V nabídce **Nástroje** klikněte na **Rozšíření a aktualizace**.
...
5. Znovu sestavte soubor VSIX a otevřete ho v Průzkumníkovi Windows, abyste ho znovu nainstalovali. Restartujte [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] .
   ...
   1. Odebrat soubor rozšíření z podsložky *localappdata*\Microsoft\VisualStudio \\ [Version] \Extensions\
