...
V textové šabloně v [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] můžete vložit text z jiného souboru pomocí direktivy `<#@include#>`. Můžete umístit direktivy `include` kdekoli v textové šabloně před tím, `<#+ ... #>` blok funkce první třídy. Zahrnuté soubory mohou obsahovat také direktivy `include` a další direktivy. Díky tomu můžete kód šablony a často používaný text sdílet mezi šablonami.
...
- `filePath` může být absolutní nebo relativní k aktuálnímu souboru šablony.
   ...
   Kromě toho můžou konkrétní rozšíření [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] určovat vlastní adresáře pro hledání souborů k zahrnutí. Pokud jste například nainstalovali sadu pro vizualizaci a modelování sady SDK (DSL Tools), přidá se do seznamu zahrnutí následující složka: `Program Files\Microsoft Visual Studio 10.0\Common7\IDE\Extensions\Microsoft\DSL SDK\DSL Designer\11.0\TextTemplates`.
...
- Název zahrnutého souboru nemusí používat rozšíření `".tt"`.
   ...
   Pro zahrnuté soubory můžete chtít použít jinou příponu, například `".t4"`. Důvodem je, že když do projektu přidáte `.tt` soubor, [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] automaticky nastaví jeho vlastnost **vlastní nástroj** na `TextTemplatingFileGenerator`. Vkládané soubory obvykle nechcete transformovat individuálně.
