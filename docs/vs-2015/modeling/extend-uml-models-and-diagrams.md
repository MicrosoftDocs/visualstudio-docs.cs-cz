...
## <a name="model-bus"></a>Sběrnice modelu
 Sběrnice modelů umožňuje přístup k diagramu nebo modelu z jiného diagramu nebo z jiného rozšíření [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)]. Mimo jiné to umožňuje rozložit informace na více než jeden model, aby více uživatelů mohl pracovat současně na kombinovaném modelu.
