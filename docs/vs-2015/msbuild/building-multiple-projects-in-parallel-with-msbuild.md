...
## <a name="buildinparallel-task-parameter"></a>Parametr úlohy BuildInParallel  
 `BuildInParallel` je volitelný logický parametr u úlohy [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)]. Pokud je `BuildInParallel` nastaveno na `true` (výchozí hodnota), vygeneruje se více pracovních procesů, aby se sestavilo co nejvíce projektů. Aby tento postup správně fungoval, musí být přepínač `/maxcpucount` nastaven na hodnotu větší než 1 a systém musí být alespoň dvoujádrový nebo mít dva nebo více procesorů.  
...
## <a name="see-also"></a>Viz také  
 ...
 [Blog C++ o optimalizaci sestavení paralelně](https://blogs.msdn.com/b/visualstudio/archive/2010/03/08/tuning-c-build-parallelism-in-vs2010.aspx)
