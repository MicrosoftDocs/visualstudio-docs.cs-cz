...
ms.locfileid: "64816213"
---
# <a name="generateapplicationmanifest-task"></a>GenerateApplicationManifest – úloha
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

Generuje [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] manifest aplikace nebo nativní manifest. Nativní manifest popisuje komponentu tak, že definuje jedinečnou identitu pro komponentu a identifikuje všechna sestavení a soubory, které tvoří komponentu. [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)]Manifest aplikace rozšiřuje nativní manifest tím, že označuje vstupní bod aplikace a určuje úroveň zabezpečení aplikace.  
...
|`Dependencies`|Volitelný <xref:Microsoft.Build.Framework.ITaskItem> `[]` parametr.<br /><br /> Určuje seznam položek, který definuje sadu závislých sestavení pro generovaný manifest. Každá položka může být dále popsána metadaty položky k označení dalšího stavu nasazení a typu závislosti. Další informace najdete níže v části metadata položky.|  
|`Description`|Volitelný `String` parametr.<br /><br /> Určuje popis aplikace nebo komponenty.|  
|`EntryPoint`|Volitelný <xref:Microsoft.Build.Framework.ITaskItem> `[]` parametr.<br /><br /> Určuje jednu položku, která označuje vstupní bod pro vygenerované sestavení manifestu.<br /><br /> Pro [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] manifest aplikace tento parametr určuje sestavení, které se spustí při spuštění aplikace.|  
|`ErrorReportUrl`|Volitelné [řetězec] (<!-- TODO: review code entity reference <xref:assetId:///String?qualifyHint=False&amp;autoUpgrade=True>  -->ukazatele.<br /><br /> Určuje adresu URL webové stránky, která se zobrazí v dialogových oknech během hlášení chyb v instalacích ClickOnce.|  
...
|`HostInBrowser`|Volitelné [Boolean] (<!-- TODO: review code entity reference <xref:assetId:///Boolean?qualifyHint=False&amp;autoUpgrade=True>  -->ukazatele.<br /><br /> Pokud `true` je aplikace hostována v prohlížeči (stejně jako aplikace WPF pro webový prohlížeč).|  
...
|`TargetFrameworkMoniker`|Volitelné <!-- TODO: review code entity reference <xref:assetId:///String?qualifyHint=False&amp;autoUpgrade=True>  --> ukazatele.<br /><br /> Určuje moniker cílového rozhraní .NET Framework.|  
|`TargetFrameworkProfile`|Volitelné <!-- TODO: review code entity reference <xref:assetId:///String?qualifyHint=False&amp;autoUpgrade=True>  --> ukazatele.<br /><br /> Určuje profil cílového rozhraní .NET Framework.|  
|`TargetFrameworkSubset`|Volitelné <!-- TODO: review code entity reference <xref:assetId:///String?qualifyHint=False&amp;autoUpgrade=True>  --> ukazatele.<br /><br /> Určuje název podmnožiny .NET Framework, na kterou se má cílit.|  
|`TargetFrameworkVersion`|Volitelné <!-- TODO: review code entity reference <xref:assetId:///String?qualifyHint=False&amp;autoUpgrade=True>  --> ukazatele.<br /><br /> Určuje cílovou .NET Framework projektu.|  
...
|`UseApplicationTrust`|Volitelné <!-- TODO: review code entity reference <xref:assetId:///Boolean?qualifyHint=False&amp;autoUpgrade=True>  --> ukazatele.<br /><br /> Pokud je hodnota true `Product` , `Publisher` vlastnosti, a `SupportUrl` jsou zapsány do manifestu aplikace.|  
...
## <a name="example"></a>Příklad  
 Tento příklad používá `GenerateApplicationManifest` úlohu k vygenerování [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] manifestu aplikace a `GenerateDeploymentManifest` úlohu pro vygenerování manifestu nasazení pro aplikaci s jedním sestavením. Potom používá `SignFile` úlohu k podepsání manifestů.  
  
 To ukazuje nejjednodušší možný scénář generování manifestu, ve kterém [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] jsou vygenerovány manifesty pro jeden program. Výchozí název a identita jsou odvozeny ze sestavení pro manifest.  
  
> [!NOTE]
> V následujícím příkladu jsou všechny binární soubory aplikace předem připravené, aby se daly zaměřit na aspekty vytváření manifestu. Tento příklad vytvoří plně funkční [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] nasazení.  
...
## <a name="example"></a>Příklad  
 Tento příklad používá `GenerateApplicationManifest` úlohy a `GenerateDeploymentManifest` k vygenerování [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] manifestů aplikace a nasazení pro aplikaci s jedním sestavením a určením názvu a identity manifestů.  
...
## <a name="example"></a>Příklad  
 Tento příklad využívá `GenerateApplicationManifest` úlohy a `GenerateDeploymentManifest` ke generování [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] manifestů aplikace a nasazení pro aplikaci s více soubory a sestaveními.  
...
## <a name="example"></a>Příklad  
 ...
 V tomto příkladu se vytvoří Test.exe. manifest, což způsobí, že nástroj XCOPY aplikace bude nasazený s využitím bezplatné registrace modelu COM.  
