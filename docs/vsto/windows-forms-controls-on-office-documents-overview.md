...
# <a name="windows-forms-controls-on-office-documents-overview"></a>Přehled model Windows Formsch ovládacích prvků v dokumentech Office
 ...
 Další informace najdete v tématu [Postup: přidání model Windows Formsch ovládacích prvků do dokumentů Office](../vsto/how-to-add-windows-forms-controls-to-office-documents.md).
...
### <a name="layout-style-of-controls-on-a-word-document"></a>Styl rozložení ovládacích prvků v dokumentu aplikace Word
 ...
 Když přidáte ovládací prvek do dokumentu aplikace Word v době běhu, můžete určit styl rozložení nového ovládacího prvku pomocí různých `Add` \<*control class*> přetížení metod <xref:Microsoft.Office.Tools.Word.ControlCollection> třídy:

- Chcete-li přidat ovládací prvek na řádku s textem, použijte přetížení, které přijímá a <xref:Microsoft.Office.Interop.Word.Range> Určuje umístění ovládacího prvku.
