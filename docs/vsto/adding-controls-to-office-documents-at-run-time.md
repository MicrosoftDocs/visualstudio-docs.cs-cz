...
- [Přidejte model Windows Forms ovládací prvky do dokumentů](#WindowsForms).
  ...
  ![odkaz na video](../vsto/media/playvideo.gif "odkaz na video") Související video ukázku najdete v tématu [návody: Přidat ovládací prvky na plochu dokumentu v době běhu? ](http://go.microsoft.com/fwlink/?LinkId=132782).

## <a name="ControlsCollection"></a>Správa ovládacích prvků v době běhu pomocí kolekcí ovládacích prvků
 Chcete-li přidat, získat nebo odebrat ovládací prvky v době běhu, použijte pomocné <xref:Microsoft.Office.Tools.Excel.ControlCollection> metody <xref:Microsoft.Office.Tools.Word.ControlCollection> a objekty.

 Způsob, jakým přistupujete k těmto objektům, závisí na typu projektu, který vyvíjíte:

- V projektu na úrovni dokumentu pro aplikaci Excel <xref:Microsoft.Office.Tools.Excel.Worksheet.Controls%2A> použijte vlastnost `Sheet1`třídy, `Sheet2`a `Sheet3` . Další informace o těchto třídách naleznete v tématu [položka hostitele na listu](../vsto/worksheet-host-item.md).
...
### <a name="access-and-delete-controls"></a>Ovládací prvky pro přístup a odstranění
 ...
 Dynamické ovládací prvky lze odebrat voláním `Delete` metody ovládacího prvku nebo `Remove` voláním metody každé kolekce ovládacích prvků. Následující příklad kódu používá <xref:Microsoft.Office.Tools.Excel.ControlCollection.Remove%2A> metodu pro <xref:Microsoft.Office.Tools.Excel.NamedRange> odebrání z `Sheet1` v projektu na úrovni dokumentu v aplikaci Excel.
...
> [!NOTE]
> Neodstraňujte programově ovládací prvky v `Shutdown` obslužné rutině události v dokumentu. Prvky uživatelského rozhraní dokumentu již nejsou při `Shutdown` vyvolání události k dispozici. Chcete-li odebrat ovládací prvky před zavřením dokumentu, přidejte kód do obslužné rutiny události pro jinou událost <xref:Microsoft.Office.Tools.Word.Document.BeforeClose> , například nebo <xref:Microsoft.Office.Tools.Word.Document.BeforeSave> pro <xref:Microsoft.Office.Tools.Excel.Workbook.BeforeSave> Word, <xref:Microsoft.Office.Tools.Excel.Workbook.BeforeClose>nebo pro aplikaci Excel.
...
> [!NOTE]
> Pomocné metody nejsou k dispozici pro následující hostitelské ovládací prvky, protože tyto ovládací prvky nelze programově přidat do dokumentů <xref:Microsoft.Office.Tools.Excel.XmlMappedRange>: <xref:Microsoft.Office.Tools.Word.XMLNode>, <xref:Microsoft.Office.Tools.Word.XMLNodes>a.
...
- V případě aplikace Word použijte jednu z <xref:Microsoft.Office.Tools.Word.ControlCollection.AddControl%2A> metod <xref:Microsoft.Office.Tools.Word.ControlCollection> objektu.
  ...
  Chcete-li přidat ovládací prvek, <xref:System.Windows.Forms.Control>předejte rozhraní, umístění ovládacího prvku a název, který jednoznačně identifikuje ovládací prvek `AddControl` pro metodu. `AddControl` Metoda vrátí objekt, který definuje, jak ovládací prvek komunikuje s listem nebo dokumentem. Metoda vrátí (pro Excel) nebo objekt (pro Word). <xref:Microsoft.Office.Tools.Word.ControlSite> `AddControl` <xref:Microsoft.Office.Tools.Excel.ControlSite>

  Následující příklad kódu ukazuje, jak použít <xref:Microsoft.Office.Tools.Excel.ControlCollection.AddControl%2A> metodu k dynamickému Přidání vlastního uživatelského ovládacího prvku do listu v projektu aplikace Excel na úrovni dokumentu. V tomto příkladu je uživatelský ovládací prvek pojmenován `UserControl1` <xref:Microsoft.Office.Interop.Excel.Range> a má název `range1`. Chcete-li použít tento příklad, spusťte jej `Sheet`z třídy *n* v projektu.
...
- Objekt, nebo`OLEControl` , který představuje ovládací prvek po jeho přidání do listu nebo dokumentu. `ControlSite` `OLEObject`
...
- Pro přístup ke členům, kteří patří pouze do vlastního ovládacího prvku, <xref:System.Windows.Forms.Control>použijte.

- Pro přístup ke členům, které jsou sdíleny ovládacími prvky `ControlSite`, `OLEObject`použijte objekt `OLEControl` , nebo.

  Pokud přistupujete ke sdílenému členu z <xref:System.Windows.Forms.Control>, může selhat bez upozornění nebo oznámení nebo může způsobit neplatné výsledky. Vždy používejte `ControlSite`metody nebo vlastnosti objektu, `OLEObject`nebo `OLEControl` , pokud není požadovaná metoda nebo vlastnost k dispozici; pouze pak byste měli odkazovat na <xref:System.Windows.Forms.Control>.

  Například <xref:Microsoft.Office.Tools.Excel.ControlSite> třída `Top` i třída mají vlastnost. <xref:System.Windows.Forms.Control> Chcete-li získat nebo nastavit vzdálenost mezi horním okrajem ovládacího prvku a horním okrajem dokumentu, <xref:Microsoft.Office.Tools.Excel.ControlSite.Top%2A> použijte vlastnost <xref:Microsoft.Office.Tools.Excel.ControlSite>, <xref:System.Windows.Forms.Control>nikoli <xref:System.Windows.Forms.Control.Top%2A> vlastnost.
