...
* Projekt ASP.NET, ASP.NET Core, .NET Core nebo Node.js. Pokud projekt ještě nemáte, vyberte některou z níže uvedených možností:
  * ASP.NET Core: Sledujte [rychlý Start: pomocí sady Visual Studio vytvořte svou první webovou aplikaci ASP.NET Core](../../ide/quickstart-aspnet-core.md)nebo použijte **soubor**  >  **Nový projekt**, vyberte **Visual C#**  >  **.NET Core**a pak vyberte **ASP.NET Core webová aplikace**. Po zobrazení výzvy vyberte šablonu **Webová aplikace (model-zobrazení-kontroler)** , ujistěte se, že není vybráno **žádné ověřování** , a pak vyberte **OK**.
