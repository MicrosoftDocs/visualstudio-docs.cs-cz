...
Maximální počet jednotek překladu, které budou v jednom okamžiku aktivní pro požadavky IntelliSense. Je nutné zadat hodnotu mezi 2 a 15. Toto číslo přímo souvisí s maximálním počtem VCPkgSrv.exe procesů, které budou spuštěny (pro danou instanci sady Visual Studio). Výchozí hodnota je 2, ale pokud máte dostupnou paměť, můžete tuto hodnotu zvýšit a pravděpodobně dosáhnout mírného lepšího výkonu technologie IntelliSense.
...
Klíčová slova jazyka `void` , například, `class` , se `switch` nezobrazují v návrzích na seznam členů.
