...
- Pro [!INCLUDE[vsvss](../../extensibility/includes/vsvss_md.md)] otevřít ze správy zdrojového kódu: **soubor**, **otevřít**, **projekt** /**řešení**; Podívejte se na [!INCLUDE[vsvss](../../extensibility/includes/vsvss_md.md)] umístění.
...
- Pro [!INCLUDE[vsvss](../../extensibility/includes/vsvss_md.md)] je také možné přidat ze správy zdrojového kódu ze **souboru**, **Přidat**, **existující projekt**; Podívejte se na [!INCLUDE[vsvss](../../extensibility/includes/vsvss_md.md)] umístění.
...
- Když je projekt přidán do správy zdrojových kódů, je vytvořen odpovídající soubor \<*ProjectName*>. vspscc (soubor s nápovědou projektu). Obsahuje seznam souborů vyloučení a informace o připojení. Tento soubor neodstraňujte, protože obsahuje informace specifické pro projekt.

- Když je do správy zdrojového kódu přidáno řešení, je vytvořen odpovídající*soubor \< >* . vssscc (troj S). Textový soubor obsahuje informace o připojení a seznam souborů vyloučení, podobně jako soubor nápovědy k projektu. Tento soubor je dočasný a existuje pouze v databázi správy zdrojů.

- Je-li řešení otevřeno ze správy zdrojového kódu, je*soubor \< >* . vsscc (s dvojitou příponou), který existuje pouze v databázi správy zdrojů, vytvořen místně v dočasném souboru. Tento soubor obsahuje cestu ze složky pro připojení řešení k souboru řešení. Tento soubor je dočasný a po dokončení operace "otevřít ze správy zdrojového kódu" je místní kopie odstraněna.
