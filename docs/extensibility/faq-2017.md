...
Pokud použijete `PackageRegistrationAttribute`, mělo by to být automatické.
...
Ano. K dispozici je balíček NuGet, který obsahuje potřebné nástroje a úlohy pro sestavení manifestů formátu VSIX V3 v aplikaci Visual Studio 2015. Do projektu rozšiřitelnosti přidejte odkaz na balíček NuGet `Microsoft.VisualStudio.Sdk.BuildTasks.14.0`. Také budete muset přidat `VsixType` element určující `v3` do souboru projektu:
...
Ne, vlastnost VSIX V3 `InstallRoot` neumožňuje instalovat cokoli mimo strukturu instalačních složek sady Visual Studio. Podporovaná umístění najdete v tématu věnovaném [instalaci mimo složku rozšíření](set-install-root.md) .
