---
description: Načte rozložení místních proměnných pro sadu metod.
...
`cMethods`\
pro Počet tokenů metod v `rgMethodTokens` poli.

`rgMethodTokens`\
pro Pole tokenů metod

`pStreamLayout`\
mimo Textový Stream, který obsahuje rozložení proměnné.
