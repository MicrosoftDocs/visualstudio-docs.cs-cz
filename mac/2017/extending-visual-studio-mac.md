...
Balíčky rozšíření ukládají metadata týkající se jejich názvu, verze, závislostí a dalších informací v atributech jazyka C#. Tvůrce doplňku vytvoří dva soubory `AddinInfo.cs` a `AssemblyInfo.cs` uloží a uspořádá tyto informace. Balíčky rozšíření musí mít jedinečné ID a obor názvů, které jsou zadané v *atributu AddIn*:
...
* **defaultHandler** – Určuje `CommandHandler` třídu, která je založená na příkazu.
