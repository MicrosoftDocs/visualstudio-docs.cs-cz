...
- **Aplikace serveru .NET Core > Blazor**
- **Blazor aplikace WebAssembly pro .NET Core >**
...
Otevřete soubor **program.cs** a prozkoumejte ho v projektu. Všimněte si, že v metodě se děje několik věcí `Main` – vstup do vaší aplikace:
...
Spouštěcí třída vaší aplikace je určena v `UseStartup()` metodě na `CreateWebHostBuilder` . Je v této třídě, kterou určíte kanál pro zpracování požadavků a kde nakonfigurujete nějaké služby.
...
Po spuštění projektu by měl být výchozí webový prohlížeč spuštěn a připojen k adrese URL uvedené ve výstupu aplikace. Alternativně můžete otevřít libovolný prohlížeč, a to tak, že zadáte a `http://localhost:5000/` nahradíte `5000` port, který je ve výstupu aplikace Visual Studio. Měl by se zobrazit text `Hello World!` :
